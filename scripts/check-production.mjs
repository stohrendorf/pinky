...
try {
    ...
    assert.deepEqual(edited.conductor.tempos.map(marker => [marker.step, marker.bpm]), [[0, 120], [16, 60]]);
    ...
    await page.waitForFunction(() => window.testProgress.some(state => state.stage === 'rendering' && state.progress > 0));
    ...
    const stopped = await page.evaluate(async () => {
        ...
        return {state: context.state, at, later: context.currentTime, rendering: engine.isRendering()};
    });
    ...
    assert.equal(wav.readUInt32LE(40), expectedFrames * 4, 'WAV length includes integrated tempo ramp + tails');
    ...
    for (const stage of ['scheduling', 'rendering', 'encoding']) {
        ...
        assert.ok(values.every((value, index) => index === 0 || value >= values[index - 1]), `${stage} never regresses`);
    }
    ...
    assert.deepEqual(fallbackStopped, {state: 'closed', rendering: false, cancelled: true, encoded: false});
    ...
    const fallbackProgress = await page.evaluate(() => window.testProgress.filter(state => state.stage === 'rendering').map(state => state.progress));
