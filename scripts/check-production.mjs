...
try {
  ...
  const addMarker = async (step) => {
    ...
    assert.equal(
      await conductor
        .getByLabel("Exact step (0-based)", { exact: true })
        .count(),
      0,
    );
  };
  ...
  assert.equal(
    await page.locator(".conductor select").count(),
    0,
    "no marker picker",
  );
  assert.equal(
  ...
  assert.equal(
    await conductor.getByLabel("BPM", { exact: true }).inputValue(),
    "120",
  );
  assert.equal(
    await conductor
      .getByLabel("Gradually change to the next tempo")
      .isChecked(),
    true,
  );
  assert.equal(
    await conductor.getByLabel("Time signature", { exact: true }).inputValue(),
    "7/8",
  );
  assert.equal(
    await conductor.getByLabel("Title", { exact: true }).inputValue(),
    "First rain",
  );
  assert.equal(
    await conductor
      .getByRole("button", { name: /Jump here|Loop section|Clear loop/ })
      .count(),
    0,
  );
  assert.equal(
    await conductor
      .getByRole("group", { name: "Markers at this position" })
      .count(),
    0,
  ...
  assert.equal(
    edited.loop ?? null,
    null,
    "opening markers leaves the loop alone",
  );
  assert.deepEqual(
    edited.patterns,
    edited.original,
    "meter changes must not move notes",
  );
  assert.deepEqual(
  ...
  const lane = await page.locator(".conductor-lanes").boundingBox();
  const barWidth = (
    await page.getByTitle("Bar 1 · 7/8 · step 0", { exact: true }).boundingBox()
  ).width;
  ...
  assert.equal(
    await conductor.count(),
    0,
    "clicking the lane only creates a placeholder",
  );
  await placeholder.click();
  await page.waitForFunction(
    () =>
      document.querySelector(".conductor-editor .marker-value") ===
  ...
  const firstBar = await page
    .getByTitle("Bar 1 · 7/8 · step 0", { exact: true })
    .boundingBox();
  const secondBar = await page
    .getByTitle("Bar 2 · 7/8 · step 14", { exact: true })
    .boundingBox();
  ...
  await page.getByRole("button", { name: /^Edit First rain/ }).click();
  assert.equal(
    await conductor.getByLabel("Title", { exact: true }).isDisabled(),
    true,
  );
  assert.equal(
    await conductor
  ...
  await page.waitForFunction(() =>
    window.testProgress.some(
      (state) => state.stage === "rendering" && state.progress > 0,
    ),
  );
  assert.equal(
  ...
  assert.equal(
    downloads.length,
    0,
    "cancel must never download a partial file",
  );
  ...
  const expectedFrames = await page.evaluate(async () => {
    ...
    return Math.ceil(
      (createTimingMap(p).secondsBetween(0, 32) +
        3 +
        1.5 * p.instruments[0].params.rel) *
        44100,
    );
  });
  ...
  assert.equal(
    downloads.length,
    1,
    "encoding cancellation also suppresses downloads",
  );
  ...
  await page.waitForFunction(() =>
    window.testProgress.some(
      (state) =>
        state.stage === "rendering" && state.progress > 0 && state.progress < 1,
    ),
  );
  assert.notEqual(
  ...
  assert.equal(
    downloads.length,
    1,
    "fallback cancellation discards the completed render",
  );
  await page.getByRole("button", { name: "Play pattern", exact: true }).click();
  await page.getByRole("button", { name: "Mixer", exact: true }).click();
  await page.waitForFunction(
  ...
  const fallbackDownloadEvent = page.waitForEvent("download", {
    timeout: 30000,
  });
  await exportButton.click();
  ...
  const measured = fallbackProgress.filter(
    ...
    JSON.stringify(
      {
        ...
        export: {
          ...
          checks:
            "native suspension, missing pause APIs, focus, Escape, live playback recovery, retry, encoding cancel",
        },
      },
      null,
      2,
