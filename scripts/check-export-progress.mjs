...
try {
    ...
    const expectedFrames = await page.evaluate(async steps => {
        ...
        p.patterns[0].tracks[id] = Array.from({ length: 8 }, (_, i) =>
            ['C5', 'E5', 'G5', 'B5'].map(pitch => ({ pitch, start: i * 4, len: 3 })),
        ).flat();
    ...
    assert.ok(
        rendered.some(
            state => state.nativeState === 'running' && state.progress > 0 && state.progress < 1,
        ),
        ...
        JSON.stringify(
            {
                ...
                wavBytes: wav.length,
                cancelled,
                checks: 'in-flight frames, ETA, WAV duration/audio, Escape, no cancelled download, playback, retry',
            },
            null,
            2,
        ),
