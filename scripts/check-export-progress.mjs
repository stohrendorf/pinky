...
assert.ok(['firefox', 'chromium'].includes(browserName), 'Choose firefox or chromium');
...
try {
    ...
    assert.equal(wav.length, 44 + expectedFrames * 4, 'exact duration, including tails and latency trim');
    assert.ok(wav.subarray(44).some(value => value !== 0), 'real engine audio, not just progress');
    const rendered = await page.evaluate(() => window.testProgress.filter(state => state.stage === 'rendering'));
    const values = rendered.filter(state => typeof state.progress === 'number').map(state => state.progress);
    assert.ok(values.some(value => value > 0.1 && value < 0.9), 'meaningful intermediate audio frames');
    assert.ok(values.every((value, i) => i === 0 || value >= values[i - 1]), 'no percentage regressions');
    assert.equal(values.at(-1), 1);
    assert.ok(rendered.some(state => state.nativeState === 'running' && state.progress > 0 && state.progress < 1),
    ...
    assert.match(await dialog.locator('.status').innerText(), /waiting for this browser to finish/);
