...
try {
    ...
    await dialog.getByRole('button', { name: `${state.name} settings`, exact: true }).click();
    assert.equal(
        await dialog.getByLabel(`${state.name} output`, { exact: true }).inputValue(),
        'master',
    );
    ...
    const result = await page.evaluate(async () => {
        ...
        check(
            groupLevel.peak / dryLevel.peak > 0.45 && groupLevel.peak / dryLevel.peak < 0.55,
        ...
        check(
            !engine.isRendering() && JSON.stringify(saved) === JSON.stringify(engine.master),
