...
try {
  ...
  await page
    .getByRole("button", { name: "Edit instrument", exact: true })
    .click();
  const instrumentEditor = page.getByRole("dialog", {
    name: "Instrument editor",
    exact: true,
  });
  await instrumentEditor.waitFor();
  assert.equal(
    await instrumentEditor.getByText("Range C4–B6", { exact: true }).count(),
    1,
  );
  await page.evaluate(() =>
  ...
  await page.waitForFunction(
    async (id) =>
      ((await import("/src/lib/engine.ts")).mixerMeters().channels[id]?.peak ??
        0) > 0.0001,
    initial.id,
    { timeout: 5000 },
  );
  ...
  await instrumentEditor
    .getByRole("button", { name: "Octave up", exact: true })
    .click();
  assert.equal(
    await instrumentEditor.getByText("Range C5–B7", { exact: true }).count(),
    1,
  );
  await page.keyboard.press("Escape");
  ...
  await page
    .getByRole("button", { name: "Add automation lane", exact: true })
    .click();
  ...
  assert.equal(
    await automation
      .getByRole("button", { name: "Master FX", exact: true })
      .count(),
    1,
  );
  await page.keyboard.press("Escape");
  ...
  assert.ok(
    ...
    "compact mixer does not reserve a tall empty panel",
  );
  if (process.argv.includes("--screenshot")) {
    await page.screenshot({ path: join(root, "mixer-simple-check.png") });
  }

  const fader = dialog.getByRole("slider", {
    name: `${initial.name} fader`,
    exact: true,
  });
  ...
  assert.equal(
    Number(await fader.inputValue()),
    0.01,
    "up raises the fader in this browser",
  );
  await page.keyboard.press("End");
  assert.equal(Number(await fader.inputValue()), 2);
  assert.equal(
    await page.evaluate(
      (id) => window.mixerSong.mixer.channels[id].volume,
      initial.id,
    ),
    2,
  );
  ...
  await dialog
    .getByRole("button", { name: `Mute ${initial.name}`, exact: true })
    .click();
  await dialog
    .getByRole("button", { name: `Solo ${initial.name}`, exact: true })
    .click();
  assert.deepEqual(
    await page.evaluate((id) => {
  ...
  assert.equal(
    await panel
      .getByRole("slider", { name: "High-pass", exact: true })
      .isVisible(),
    false,
  );
  await tone.click();
  const compressor = panel.getByRole("checkbox", {
    name: "Compressor",
    exact: true,
  });
  await compressor.uncheck();
  assert.equal(
    await panel.getByRole("slider", { name: "Threshold", exact: true }).count(),
    0,
  );
  ...
  await panel
    .getByRole("slider", { name: "Reverb send", exact: true })
    .press("End");
  assert.equal(
    await page.evaluate(
      (id) => window.mixerSong.mixer.channels[id].reverb,
      initial.id,
    ),
    1,
  );
  ...
  await dialog
    .getByRole("button", { name: "+ Delay return", exact: true })
    .click();
  await panel
    .getByRole("textbox", { name: "Bus name", exact: true })
    .fill("Test echo");
  await page.keyboard.press("Tab");
  const bus = await page.evaluate(() => window.mixerSong.mixer.buses.at(-1).id);
  await panel
    .getByRole("slider", { name: "Delay time", exact: true })
    .press("End");
  assert.equal(
    await page.evaluate(() => window.mixerSong.mixer.buses.at(-1).delayTime),
    2,
  );
  await heading.click();
  assert.equal(
  ...
  await panel
    .getByRole("slider", { name: "Send to Test echo", exact: true })
    .press("End");
  assert.equal(
    await page.evaluate(
      ({ id, bus }) =>
        window.mixerSong.mixer.channels[id].sends.find((s) => s.busId === bus)
          .level,
      { id: initial.id, bus },
    ),
    1,
  ...
  await panel
    .getByLabel(`${initial.name} output`, { exact: true })
    .selectOption(bus);
  await dialog
    .getByRole("button", { name: "Test echo settings", exact: true })
    .click();
  ...
  await panel
    .getByRole("slider", { name: "Ceiling", exact: true })
    .press("Home");
  assert.equal(
    await page.evaluate(() => window.mixerSong.mixer.master.ceilingDb),
    -12,
  );
  await dialog.getByRole("button", { name: "Limiter", exact: true }).click();
  assert.equal(
    await page.evaluate(() => window.mixerSong.mixer.master.limiter),
    false,
  );
  await panel
    .getByRole("button", { name: "Close channel settings", exact: true })
    .click();
  await dialog
    .getByRole("button", { name: "Master settings", exact: true })
    .click();
  ...
  const master = dialog.getByRole("region", {
    name: "Master strip",
    exact: true,
  });
  ...
  console.log(
    JSON.stringify({
      ...
      checks:
        "preview audio, QWERTZ layout, logo menu, automation trees/mixer lane, compact mixer, faders, mute/solo, history, routing, delay, limiter, focus",
    }),
  );
} finally {
  await browser?.close();
