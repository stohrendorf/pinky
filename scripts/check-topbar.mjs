...
try {
  ...
  assert.equal(
    await page.locator("a.brand, button.brand, .brand[tabindex]").count(),
    0,
  );
  assert.equal(
    await page
  ...
  const saveWidth = await save.evaluate(
    (node) => node.getBoundingClientRect().width,
  );
  for (let i = 0; i < 2; i++) {
    await save.click();
    await page.waitForFunction(() =>
      document.querySelector(".save-icon .fa-check"),
    );
    assert.equal(await save.locator(".save-icon").count(), 1);
    assert.equal(
      await save.evaluate((node) => node.getBoundingClientRect().width),
      saveWidth,
    );
    await page.waitForFunction(
      () => document.querySelector(".saved-flash").textContent === "",
    );
  ...
  assert.equal(
    await page.evaluate(() => JSON.stringify(window.readToolbarState())),
    baseline,
  );
  await page.keyboard.press("Escape");
  await panel.waitFor({ state: "hidden" });
  assert.equal(
    await demos.evaluate((node) => node === document.activeElement),
    true,
  );
  ...
  await demos.click();
  await panel
    .getByRole("button", { name: "Restore previous project", exact: true })
    .click();
  await panel.waitFor({ state: "hidden" });
  assert.equal(
    await page.evaluate(() => JSON.stringify(window.readToolbarState())),
  ...
  await page
    .getByRole("spinbutton", { name: "Tempo (BPM)", exact: true })
    .click();
  assert.equal(await page.getByRole("dialog").count(), 0);

  const audition = page.getByRole("button", {
    name: "Play pattern",
    exact: true,
  });
  assert.equal(
    await page
      .locator(".topbar")
      .getByRole("button", { name: "Play pattern" })
      .count(),
    0,
  );
  assert.equal(
  ...
  await page.waitForFunction(
    () =>
      document
        .querySelector(".pattern-preview")
        .getAttribute("aria-pressed") === "true",
  );
  assert.equal(
    await audition.isEnabled(),
    true,
    "audition remains stoppable while playing",
  );
  ...
  await page.waitForFunction(
    () => !document.querySelector('button[aria-label="Stop"]').disabled,
  );
  assert.equal(await audition.getAttribute("aria-pressed"), "false");
  await audition.click();
  await page.waitForFunction(
  ...
  const json = page.getByRole("button", {
  ...
  assert.equal(
    await json.evaluate((node) => node === document.activeElement),
    true,
  );
  const triggerBounds = await exports.boundingBox();
  const panelBounds = await exportPanel.boundingBox();
  assert.ok(
    Math.abs(triggerBounds.x - panelBounds.x) < 2,
    "dropdown is anchored to Export",
  );
  await page.keyboard.press("ArrowDown");
  assert.equal(
    await wav.evaluate((node) => node === document.activeElement),
    true,
  );
  await page.keyboard.press("Escape");
  await exportPanel.waitFor({ state: "hidden" });
  assert.equal(
    await exports.evaluate((node) => node === document.activeElement),
    true,
  );
  await exports.click();
  await demos.click();
  await panel.waitFor();
  assert.equal(
    await exportPanel.count(),
    0,
    "only one toolbar dropdown is open",
  );
  await exports.click();
  await exportPanel.waitFor();
  ...
  await page.waitForFunction(
    () => document.activeElement?.textContent.trim() === "Export",
  );

  for (const width of [1280, 1440]) {
    await page.setViewportSize({ width, height: 900 });
    const layout = await page.evaluate(() => ({
      height: document.querySelector(".topbar-main").getBoundingClientRect()
        .height,
      clipped: [
        ...document.querySelectorAll(".topbar-main button, .topbar-main input"),
      ].filter((node) => node.getBoundingClientRect().right > innerWidth)
        .length,
      transportText: document
        .querySelector(".transport-controls")
        .textContent.trim(),
    }));
  ...
  assert.deepEqual(
    errors,
    [],
    "No browser exceptions during toolbar interactions",
  );
  console.log(
