...
try {
  ...
  const marker = (step) =>
    page.locator(`.conductor button[data-step="${step}"]`);
  ...
  assert.equal(
    await editor.getByLabel("Time signature", { exact: true }).inputValue(),
    "4/4",
  );
  ...
  await page.evaluate(async () => {
    ...
    p.conductor = {
      ...
      meters: [
        { id: crypto.randomUUID(), step: 0, numerator: 7, denominator: 8 },
      ],
    };
    model.project.set(p);
  ...
  const drag = async (from, to, { cancel = false, hold = false } = {}) => {
    ...
    assert.equal(
      await page.evaluate(
        ({ x, y }) =>
          document.elementFromPoint(x, y)?.getAttribute("data-step"),
        {
          x: box.x + 8,
    ...
    assert.equal(
      await editor.count(),
      0,
      "releasing a drag does not open the editor",
    );
  };
  ...
  assert.deepEqual(
    await data(),
    moved,
    "conflicting drop never overwrites the other tempo",
  );
  ...
  assert.equal(
    await editor.count(),
    0,
    "creation leaves a movable placeholder",
  );
  assert.deepEqual(
    (await data()).tempos,
    moved.tempos,
    "adding a marker does not change timing",
  );
  await drag(48, 24);
  ...
  await page.waitForFunction(() => {
    ...
    return (
      new DOMMatrix(getComputedStyle(lane).transform).m41 === -grid.scrollLeft
    );
  });
  await drag(27, 23);
  assert.ok(
  ...
  assert.equal(
    persisted.sections.find((item) => item.step === 23).name,
    "Moved placeholder",
  );
  assert.ok(
    await page.evaluate(async () => {
  ...
  assert.deepEqual(
    await data(),
    persisted,
    "grouped edits and moves survive save/reload",
  );
  ...
  console.log(
    JSON.stringify({
      ...
      checks:
        "Monsoon grouped fields, IDs, pointer capture, drag preview/drop, undo/redo, Escape, collisions, placeholders, arrow keys, scroll, save/reload",
    }),
  );
} finally {
  await browser?.close();
