...
try {
    ...
    assert.equal(await page.locator('.conductor .marker button').count(),
        new Set(Object.values(before).flat().map(item => item.step)).size, 'one button per musical position');
    await marker(0).click();
    assert.equal(await editor.getByLabel('Title', {exact: true}).inputValue(), before.sections[0].name);
    assert.equal(await editor.getByLabel('BPM', {exact: true}).inputValue(), String(before.tempos[0].bpm));
    ...
    for (const kind of ['sections', 'tempos', 'meters']) {
        assert.deepEqual(moved[kind].find(item => item.id === initial[kind][0].id), {...initial[kind][0], step: 8});
    }
    await page.evaluate(async () => (await import('/src/lib/history.ts')).undo());
    ...
    await drag(27, 23);
    assert.ok((await data()).sections.some(item => item.step === 23), JSON.stringify(await data()));
