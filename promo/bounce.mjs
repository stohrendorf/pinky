...
const out = resolve(
  args.find((a) => !a.startsWith("--")) || join(root, "promo", "out"),
);
const SAMPLE_RATE = 48000;

async function launchBrowser() {
  if (process.env.PINKY_BROWSER) {
    return chromium.launch({ executablePath: process.env.PINKY_BROWSER });
  }
  const errors = [];
  ...
  throw new Error(
    "no Chromium-based browser found — install Chrome or Edge, or set PINKY_BROWSER to an executable\n" +
      errors.join("\n"),
  );
}
...
try {
  ...
  if (measure) {
    ...
    for (const l of levels) {
      console.log(
        `${l.part.padEnd(8)} ${l.gain.toFixed(2).padStart(5)} ${l.refDb.toFixed(1).padStart(9)} ${l.peakDb.toFixed(1).padStart(10)}`,
      );
    }
  } else {
    ...
    console.log(
      `bounce: ${result.score.notes.length} notes, ${result.score.length.toFixed(1)} s, sample peak ${result.peakDb.toFixed(1)} dBFS -> ${wav}`,
    );
    if (result.peakDb > 0) {
      console.warn(
        "bounce: the render clips — lower the demo's master volume lane",
      );
    }
    console.log(
      `bounce: bar peaks ${result.barPeaksDb.map((v, i) => `${i}:${v.toFixed(1)}`).join(" ")}`,
    );
