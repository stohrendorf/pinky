...
export function elements(nodes: Node[], name: string): Node[] {
    return walk(nodes).filter(node => node.type === 'RegularElement' && node.name === name);
}

export function components(nodes: Node[], name: string): Node[] {
    return walk(nodes).filter(node => node.type === 'Component' && node.name === name);
}
...
export function styleRules(source: string): Map<string, Map<string, string>> {
    ...
    for (const child of stylesheet?.children ?? []) {
        ...
        rules.set(source.slice(child.prelude.start, child.prelude.end), declarations);
    }
    return rules;
}
