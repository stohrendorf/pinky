...
import {
    createMixer, defaultChannel
} from './mixer';

/** Production choices only: never rewrite the score, patches or automation. */
export function mixDemo(p: Project, profile: 'monsoon' | 'winter' | 'pocket'): Project {
    const mixer = createMixer(p.instruments.map(inst => inst.id));
    const family = {monsoon: '0001', winter: '0002', pocket: '0003'}[profile];
    const group = (name: string, effect: MixerBus['effect'] = 'none'): MixerBus => {
        const bus: MixerBus = {
            ...defaultChannel(), id: `47e4ac00-${family}-4a13-9281-${String(mixer.buses.length + 1).padStart(12, '0')}`,
            name, reverb: 0, effect, delayTime: 60 / p.bpm * 0.75, feedback: 0.25
        };
    ...
    if (profile === 'monsoon') {
        const drums = group('Taiko & hand drums'), bronze = group('Bronze ensemble');
        const voices = group('Breath & voice'), weather = group('Weather');
        const plucks = group('Kora & handpan'), echo = group('Temple echo', 'delay');
        echo.highpass = 350;
        echo.tilt = -3;
        echo.pan = -0.25;
        echo.reverb = 0.35;
        ...
        for (const inst of p.instruments) {
            const strip = mixer.channels[inst.id], name = inst.name;
            strip.reverb = 0.75;
            if (name.startsWith('Drums/') || name.endsWith('Ceng-ceng')) {
                ...
                strip.reverb = 0.7;
                strip.sends = [{busId: echo.id, level: name.endsWith('Kora') ? 0.08 : 0.06}];
            } else {
        ...
        const strings = group('Ripieno strings'), continuo = group('Continuo'), winds = group('Venti');
        winds.highpass = 80;
        for (const inst of p.instruments) {
            ...
            if (inst.name.endsWith('Solo Violin')) {
                ...
                strip.reverb = 0.75;
            } else if (inst.name.endsWith('Violone') || inst.name.endsWith('Harpsichord') || inst.name.endsWith('Violoncelli')) {
        ...
        const drums = group('Pocket drums'), harmony = group('Keys & guitar'), echo = group('Slap echo', 'delay');
