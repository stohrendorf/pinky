...
export function updateMarkerAt(
  ...
  const ids = new Set(
    [...data.tempos, ...data.meters, ...data.sections].map(
      (marker) => marker.id,
    ),
  );
  const nextId = (): string => {
...
export function moveMarkerAt(
  ...
  if (
    !kinds.some((kind) => data[kind].some((marker) => marker.step === from))
  ) {
  ...
  const move = <T extends { step: number }>(markers: T[]): T[] =>
    markers
      .map((marker) =>
        marker.step === from ? { ...marker, step: to } : marker,
      )
      .sort(byStep);
  return {
    ...data,
