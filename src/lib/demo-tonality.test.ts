...
import { analyzeNote, isPitched, measureNote, renderNote, ringTime } from './timbre-analysis';
...
const carriesOctave = (params: InstrumentParams) =>
    (params.partials ?? []).some(
        partial => Math.abs(partial.ratio - 2) < 0.05 && partial.level >= OCTAVE_CARRIER_LEVEL,
    );
