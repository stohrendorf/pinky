...
import type { InstrumentParams, Project } from "./types";
...
function playedNotes(project: Project, instrumentId: string): Played | null {
  const notes = project.patterns.flatMap(
    (pattern) => pattern.tracks[instrumentId] || [],
  );
  if (!notes.length) {
    return null;
  }
  ...
  return {
    median: byPitch[Math.floor(byPitch.length / 2)].pitch,
    steps: byLength[Math.floor(byLength.length / 2)],
  };
}
...
const isSubDrone = ({ median, steps }: Played) =>
  noteByName[median].freq < SUB_HZ && steps >= DRONE_STEPS;
const carriesOctave = (params: InstrumentParams) =>
  (params.partials ?? []).some(
    (partial) =>
      Math.abs(partial.ratio - 2) < 0.05 &&
      partial.level >= OCTAVE_CARRIER_LEVEL,
  );

describe("demo instrument tonality", () => {
  it.each(DEMO_LIBRARY.map((demo) => demo.id))(
    "renders every pitched part of %s as a pitch, not as noise",
    (song) => {
      const parts = pitchedParts(song);
      expect(parts.length).toBeGreaterThan(0);
      const subsonic = parts
        .filter(
          ({ params, played }) =>
            noteByName[played.median].freq < SUB_HZ &&
            !isSubDrone(played) &&
            !carriesOctave(params),
        )
        .map(
          ({ name, played }) =>
            `${name} @${played.median}: ${noteByName[played.median].freq.toFixed(0)} Hz`,
        );
      const report = parts
        .filter(({ played }) => !isSubDrone(played))
        .map(({ name, params, played, bpm }) => ({
          name,
          ...measureNote(params, played.median, played.steps, bpm),
        }));
      const noisy = report
        .filter((row) => row.clarity < MIN_CLARITY)
        .map((row) => `${row.name} @${row.pitch}: ${row.clarity.toFixed(2)}`);
      const buried = report
        .filter((row) => row.peakDb < MIN_PEAK_DB)
        .map((row) => `${row.name} @${row.pitch}: ${row.peakDb.toFixed(0)} dB`);
...
describe("timbre analysis model", () => {
  ...
  it("scores a hissy sub-register bass below a clean mid-register one", () => {
    ...
    expect(measureNote(subBass, "D2", 2, 108).clarity).toBeLessThan(
      measureNote(bass, "D3", 2, 108).clarity - 0.2,
    );
  });

  it("does not judge drums, clicks and risers as pitched instruments", () => {
    expect(isPitched({ ...pluck, tone: 0, noise: 1 })).toBe(false);
    expect(isPitched({ ...pluck, q: 5, noise: 0.15, pitchDrop: 21 })).toBe(
      false,
    );
