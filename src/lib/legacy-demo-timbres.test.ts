...
const expectTonalBody = (params: InstrumentParams) => {
  ...
  expect(
    params.partials?.filter((partial) => partial.level > 0).length,
...
describe("bundled demo timbres", () => {
  it("preserves Out of Noise’s concept while giving its wind resonant detail", () => {
    const wind = buildDemoProject("noise").instruments.find(
      (value) => value.params.noiseBend === 1,
    )!.params;
  ...
  it("turns Axel F’s long effects into pitched sweeps instead of noise washes", () => {
    for (const params of buildDemoProject("axelf")
      .instruments.filter((value) => value.params.noiseBend > 0)
