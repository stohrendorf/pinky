...
): Instrument => ({ id, name, color, params: ensurePartials({ ...DEFAULT_PARAMS, ...params }) });
const hits = (
    pitch: string,
    steps: number[],
    vel: (step: number, index: number) => number,
    len = 1,
): NoteSpec[] => steps.map((step, index) => [pitch, step, len, vel(step, index)]);
...
function buildInstruments(): Instrument[] {
    return [
        ...
        instrument(ID.khoomei, 'Vocals/Khoomei Drone', COLOR.khoomei, {
            tone: 1,
            q: 26,
            partials: [1, 0.9, 0.85, 0.8, 0.8, 0.78, 0.76, 0.74, 0.72, 0.7, 0.66, 0.62].map(
                (level, index) => ({
                    ratio: index + 1,
                    level,
                }),
...
function buildPatterns(): Pattern[] {
    ...
    const patterns = [
        ...
        pattern(P.droneSeven, '03 Seven Rains/khoomei seven', SEVEN, COLOR.khoomei, {
