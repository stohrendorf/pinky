...
import { noteByName, transposePitch } from "./notes";
...
const uuid = (group: number, index: number): string =>
  `4e6b7a10-${String(group).padStart(4, "0")}-4d2c-9a1f-${String(index).padStart(12, "0")}`;
...
const notes = (...items: NoteSpec[]): Note[] =>
  items.map(([pitch, start, len = 1, vel = 0.8]) => ({
    pitch,
    start,
    len,
    vel,
...
const hits = (
  pitch: string,
  steps: number[],
  vel: (step: number, index: number) => number,
  len = 1,
): NoteSpec[] =>
...
const roll = (
  ...
  steps.map((step, index) => [
    ...
    step,
    1,
    from + ((to - from) * index) / Math.max(1, steps.length - 1),
  ]);
const accents = (
  pitch: string,
  velocities: readonly (readonly [step: number, vel: number])[],
): NoteSpec[] => velocities.map(([step, vel]) => [pitch, step, 1, vel]);
const legato = (
  patterns: Pattern[],
  patternId: string,
  instrumentId: string,
...
const kotekan = (
  stream: (string | null)[],
): { polos: NoteSpec[]; sangsih: NoteSpec[] } => {
  const polos: NoteSpec[] = [],
    sangsih: NoteSpec[] = [];
...
const pokok = (stream: (string | null)[], every: number): NoteSpec[] =>
  stream.flatMap((pitch, step): NoteSpec[] =>
    pitch && step % every === 0
      ? [
          [
            transposePitch(pitch, -12)!,
            step,
            every,
            step % (every * 4) === 0 ? 0.85 : 0.68,
          ],
        ]
      : [],
  );
const run = (pitches: string[], vel: (step: number) => number): NoteSpec[] =>
  pitches.map((pitch, step) => [pitch, step, 1, vel(step)]);
...
const odd = (count: number): number[] =>
  Array.from({ length: count / 2 }, (_, index) => index * 2 + 1);
...
function buildInstruments(): Instrument[] {
  return [
    ...
    instrument(ID.ceng, "Percussion/Ceng-ceng", COLOR.ceng, {
      ...
      partials: [
        { ratio: 1, level: 1 },
        { ratio: 1.41, level: 0.8 },
        { ratio: 2.24, level: 0.7 },
        {
          ratio: 2.83,
          level: 0.6,
        },
    ...
    instrument(ID.gong, "Percussion/Tuned/Gong Ageng", COLOR.gong, {
      ...
      partials: [
        { ratio: 1, level: 1 },
        { ratio: 2.02, level: 0.55 },
        { ratio: 2.94, level: 0.3 },
        {
          ratio: 4.1,
          level: 0.15,
        },
    ...
    instrument(ID.polos, "Percussion/Tuned/Gendér Polos", COLOR.polos, {
      ...
      partials: [
        { ratio: 1, level: 1 },
        { ratio: 2.76, level: 0.45 },
        { ratio: 5.4, level: 0.15 },
      ],
      voices: 2,
      detune: 14,
      att: 0.002,
    ...
    instrument(ID.jublag, "Percussion/Tuned/Jublag", COLOR.jublag, {
      ...
      partials: [
        { ratio: 1, level: 1 },
        { ratio: 2.76, level: 0.4 },
        { ratio: 5.4, level: 0.12 },
      ],
      voices: 2,
      detune: 10,
      att: 0.004,
    ...
    instrument(ID.khoomei, "Vocals/Khoomei Drone", COLOR.khoomei, {
      ...
      q: 26,
      partials: [
        1, 0.9, 0.85, 0.8, 0.8, 0.78, 0.76, 0.74, 0.72, 0.7, 0.66, 0.62,
      ].map((level, index) => ({
        ratio: index + 1,
        level,
      })),
...
function buildPatterns(): Pattern[] {
  const one = kotekan(STREAM_ONE),
    two = kotekan(STREAM_TWO),
    seven = kotekan(STREAM_SEVEN),
    storm = kotekan(STREAM_STORM);
  const patterns = [
    pattern(P.rain, "01 Rain/rain wash", FOUR, COLOR.rain, {
    ...
    pattern(P.ground, "01 Rain/gong and ground", FOUR, COLOR.gong, {
      [ID.gong]: [[DRONE, 0, FOUR, 1]],
      [ID.sub]: [[DRONE, 0, FOUR, 0.75]],
    }),
    pattern(P.thunder, "01 Rain/thunder", FOUR, COLOR.thunder, {
      [ID.thunder]: [["A2", 24, 4, 0.9]],
    }),
    ...
    pattern(P.drone, "01 Rain/khoomei drone", FOUR, COLOR.khoomei, {
      [ID.khoomei]: [[DRONE, 0, FOUR, 0.9]],
    }),
    ...
    pattern(P.pokokOne, "02 Bronze/pokok one", FOUR, COLOR.jublag, {
      [ID.jublag]: pokok(STREAM_ONE, 4),
    }),
    pattern(P.pokokTwo, "02 Bronze/pokok two", FOUR, COLOR.jublag, {
      [ID.jublag]: pokok(STREAM_TWO, 4),
    }),
    pattern(P.kendang, "02 Bronze/kendang groove", FOUR, COLOR.kendang, {
      ...
      [ID.shime]: hits("E5", [4, 12, 20, 27, 28], (step) =>
        step === 27 ? 0.4 : 0.55,
      ),
      [ID.ceng]: hits("A6", odd(FOUR), (step) =>
        step % 8 === 7 ? 0.65 : 0.45,
      ),
    }),
    ...
    pattern(P.odaikoRoll, "02 Bronze/odaiko roll", FOUR, COLOR.odaiko, {
      [ID.odaiko]: [
        ["A2", 0, 1, 1],
        ["A2", 8, 1, 0.7],
        ...roll("A2", [16, 18, 20, 22, 24, 25, 26, 27, 28, 29, 30, 31], 0.5, 1),
      ],
    ...
    pattern(P.taikoSeven, "03 Seven Rains/taiko seven", SEVEN, COLOR.odaiko, {
      [ID.odaiko]: hits("A2", [0, 6, 10, 14, 20, 24], (step) =>
        step % 14 === 0 ? 1 : step % 14 === 6 ? 0.75 : 0.85,
      ),
      [ID.shime]: hits("E5", [2, 4, 8, 12, 13, 16, 18, 22, 26, 27], (step) =>
        [12, 26].includes(step) ? 0.7 : [13, 27].includes(step) ? 0.45 : 0.52,
      ),
      [ID.ceng]: hits("A6", odd(SEVEN), (step) =>
        [5, 9, 19, 23].includes(step) ? 0.6 : 0.4,
      ),
    }),
    ...
    pattern(P.pokokSeven, "03 Seven Rains/pokok seven", SEVEN, COLOR.jublag, {
      [ID.jublag]: pokok(STREAM_SEVEN, 7),
    }),
    pattern(P.cascade, "03 Seven Rains/kora cascade", SEVEN, COLOR.kora, {
      [ID.kora]: run(KORA_SEVEN, (step) =>
        step % 7 === 0 ? 0.8 : step % 2 === 0 ? 0.55 : 0.48,
      ),
    }),
    ...
    pattern(
      P.droneSeven,
      "03 Seven Rains/khoomei seven",
      SEVEN,
      COLOR.khoomei,
      {
        [ID.khoomei]: [[DRONE, 0, SEVEN, 0.9]],
      },
    ),
    pattern(
      P.shakuhachiSeven,
      "03 Seven Rains/shakuhachi seven",
      SEVEN,
      COLOR.shakuhachi,
      {
        [ID.shakuhachi]: [
    ...
    pattern(P.fillSeven, "03 Seven Rains/seven fill", SEVEN, COLOR.odaiko, {
      [ID.odaiko]: [
        ...
        ["A2", 10, 1, 0.85],
        ["A2", 14, 1, 1],
        ...roll("A2", [18, 20, 22, 24, 25, 26, 27], 0.6, 1),
      ],
    ...
    pattern(P.stormTaiko, "04 Storm/storm taiko", FOUR, COLOR.odaiko, {
      ...
      [ID.shime]: hits("E5", [2, 6, 10, 14, 15, 18, 22, 26, 29, 31], (step) =>
        step === 15 ? 0.8 : step === 31 ? 0.85 : 0.5,
      ),
      [ID.kendang]: [
        ["G4", 4, 1, 0.75],
        ["D5", 7, 1, 0.45],
      ...
      [ID.ceng]: hits("A6", even(FOUR), (step) =>
        step % 4 === 0 ? 0.6 : 0.35,
      ),
    }),
    ...
    pattern(P.stormKora, "04 Storm/storm kora", FOUR, COLOR.kora, {
      [ID.kora]: run(KORA_STORM, (step) =>
        step % 8 === 0 ? 0.8 : step % 2 === 0 ? 0.62 : 0.55,
      ),
    }),
    ...
    pattern(P.pokokStorm, "04 Storm/pokok storm", FOUR, COLOR.jublag, {
      [ID.jublag]: pokok(STREAM_STORM, 8),
    }),
    ...
    pattern(P.eyeSquall, "04 Storm/eye squall", FOUR, COLOR.riser, {
      [ID.rain]: [["A5", 0, FOUR, 0.8]],
      [ID.wind]: [["A4", 0, FOUR, 1]],
      [ID.riser]: [["D6", 0, FOUR, 1]],
    }),
    pattern(P.stormDrone, "04 Storm/khoomei storm", FOUR, COLOR.khoomei, {
      [ID.khoomei]: [[DRONE, 0, FOUR, 0.95]],
    }),
...
const sectionStarts = (sections: Section[]): number[] =>
  sections.reduce<number[]>((starts, _section, index) => {
...
function buildArrangement(
  ...
  return sections.flatMap((section, index) =>
    section.layers.map(([patternId, track, transpose, span = 1], layer) => ({
...
function buildAutomation(starts: number[], end: number): AutomationLane[] {
  const at = (section: number, offset = 0): number =>
    section < starts.length ? starts[section] + offset : end;
  const lane = (
    id: string,
    target: string,
    param: string,
  ...
  const droneRoot = (section: number): string =>
    UP_SECTIONS.includes(section) ? transposePitch(DRONE, UP)! : DRONE;
  const sing = (
    phrases: Record<number, Melody>,
    value: (harmonic: number, root: string) => number,
    rest: number,
  ): AutomationPoint[] => {
    ...
    for (const [key, melody] of Object.entries(phrases)) {
      ...
      for (const [harmonic, steps] of melody) {
        points.push({
          step: at(section, offset),
          value: value(harmonic, droneRoot(section)),
          curve: "hold",
        });
        offset += steps;
      }
  ...
  const jaw = (harmonic: number, root: string): number =>
    root === DRONE
  ...
  return [
    ...
    ]),
    {
      ...
      target: ID.khoomei,
      param: "f2",
      points: sing(phrases, overtone, overtone(8)),
    },
    ...
    ]),
    {
      ...
      target: ID.khoomei,
      param: "f1",
      points: sing(phrases, jaw, overtone(3)),
    },
    lane(uuid(7, 10), ID.khoomei, "vib", [
      [0, 0, 6, "hold"],
      [24, 0, 6, "linear"],
      [25, 0, 12, "hold"],
      [29, 0, 6],
    ...
    lane(uuid(7, 21), ID.shakuhachi, "noise", [
      [0, 0, 0.2, "hold"],
      [20, 0, 0.3, "linear"],
      [25, 0, 0.4, "hold"],
      [29, 0, 0.15],
    ...
    lane(uuid(7, 22), ID.duduk, "f2", [
      [0, 0, 1350, "hold"],
      [29, 0, 1350, "smooth"],
      [29, 24, 900],
    ]),
    lane(uuid(7, 23), ID.bowl, "pan", [
      [1, 0, -0.5, "smooth"],
      [4, 0, 0.5, "hold"],
      [30, 0, 0.5, "smooth"],
      [33, 0, -0.5],
...
export function buildBronzeMonsoon(): Project {
  ...
  const tracks: Track[] = TRACK_NAMES.map((name, index) => ({
    name,
