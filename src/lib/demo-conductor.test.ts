...
import {
    buildDemoProject, isProject
} from './project';
import {
    barAt, barsInRange, createTimingMap, isConductorData
} from './timing';
import {
    songLengthSteps
} from './transport';
import {
    placeSections, WINTER_MOVEMENTS
} from './winter-demo';
...
describe('demo conductor scores', () => {
    it('labels Monsoon’s actual 7/8 bars and preserves the taiko polymeter inside 4/4', () => {
        ...
        const seven = sections.find(section => section.name === 'Seven Rains')!.step;
        ...
        expect(bars.every(bar => bar.end - bar.start === 14 && bar.numerator === 7 && bar.denominator === 8)).toBe(true);
        expect(barAt(p, storm)).toMatchObject({bar: 41, numerator: 4, denominator: 4});
        ...
        expect(barAt(p, clip.start)).toMatchObject({numerator: 4, denominator: 4});
    });

    it('gives Monsoon a restrained tempo arc, leaving clips and automation anchored to musical time', () => {
        const p = buildDemoProject('monsoon'), timing = createTimingMap(p);
        ...
        expect(p.automation!.every(lane => lane.points.every(point => point.step <= length))).toBe(true);
        expect(p.conductor!.sections.map(section => section.name)).toEqual([
            'Rain', 'Bronze', 'Seven Rains', 'Storm', 'Eye of the storm', 'Storm returns', 'After'
        ]);
    });

    it('makes every Winter section navigable without altering its established performance timing', () => {
        const p = buildDemoProject('winter'), placed = placeSections();
        expect(isProject(p)).toBe(true);
        ...
        p.conductor!.sections.forEach((marker, i) => {
            ...
            expect(marker.name).toContain(WINTER_MOVEMENTS[placed[i].movement].data.title);
            expect(p.arrangement.some(clip => clip.start === marker.step)).toBe(true);
        });
        expect(createTimingMap(p).secondsAt(songLengthSteps(p))).toBeCloseTo(songLengthSteps(p) * 60 / p.bpm / 4, 10);
