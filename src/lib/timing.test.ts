...
import { importProject, isProject, newEmptyProject } from "./project";
import {
  barAt,
  barsInRange,
  createTimingMap,
...
describe("conductor timing map", () => {
  ...
  it("holds the base before the first marker and uses the new tempo exactly at boundaries", () => {
    const p = {
      bpm: 120,
      conductor: { ...empty(), tempos: [tempo(8, 60), tempo(16, 180)] },
    };
    const map = createTimingMap(p);
  ...
  it.each([
    ...
    [120, 120],
    [120, 120.0000001],
  ])("integrates a %s→%s ramp and inverts fractional positions", (a, b) => {
    const p = {
      bpm: 112,
      conductor: { ...empty(), tempos: [tempo(0, a, "linear"), tempo(48, b)] },
    };
    const map = createTimingMap(p);
    expect(map.bpmAt(24)).toBeCloseTo((a + b) / 2);
    const expected =
      a === b ? (48 * 15) / a : (48 * 15 * Math.log(b / a)) / (b - a);
    ...
    const summed = Array.from({ length: 96 }, (_, i) =>
      map.secondsBetween(i / 2, (i + 1) / 2),
    ).reduce((a, b) => a + b, 0);
  ...
  it("does not extrapolate a ramp past its final tempo marker", () => {
    const p = {
      bpm: 112,
      conductor: { ...empty(), tempos: [tempo(0, 90, "linear")] },
    };
...
describe("meter-aware bars", () => {
  it("snaps loop edges to local beats, including a short bar and eighth-note bar endings", () => {
    const p = {
      bpm: 120,
      conductor: { ...empty(), meters: [meter(10, 7, 8), meter(38, 3, 4)] },
    };
  ...
  it("uses 4/4 by default, and real eighth-note beats in 7/8 and 3/8", () => {
    const p = {
      bpm: 120,
      conductor: { ...empty(), meters: [meter(32, 7, 8), meter(60, 3, 8)] },
    };
    expect(barAt(p, 31)).toMatchObject({ bar: 2, start: 16, end: 32, beat: 4 });
    expect(barAt(p, 44)).toMatchObject({
      bar: 3,
      start: 32,
      end: 46,
      beat: 7,
    ...
    expect(
      barsInRange(p, 30, 67).map((bar) => [bar.bar, bar.start, bar.end]),
    ).toEqual([
      [2, 16, 32],
      [3, 32, 46],
      [4, 46, 60],
  ...
  it("shortens an existing bar without moving notes when a marker lands inside it", () => {
    const p = {
      bpm: 120,
      conductor: { ...empty(), meters: [meter(10, 5, 16)] },
    };
...
describe("conductor persistence and validation", () => {
  ...
  it("rejects malformed, duplicate, unordered and excessive marker data", () => {
    ...
    const invalid: unknown[] = [
      ...
      {},
      { ...data, tempos: null },
      { ...data, tempos: [tempo(-1, 120)] },
      { ...data, tempos: [tempo(0.5, 120)] },
      { ...data, tempos: [tempo(Infinity, 120)] },
      { ...data, tempos: [tempo(0, NaN)] },
      { ...data, tempos: [tempo(0, 0)] },
      { ...data, tempos: [{ ...tempo(0, 120), curve: "smooth" }] },
      { ...data, tempos: [tempo(32, 120), tempo(0, 140)] },
      { ...data, tempos: [tempo(0, 120), tempo(0, 140)] },
      {
        ...data,
        tempos: [data.tempos[0], { ...data.tempos[1], id: data.tempos[0].id }],
      },
      { ...data, meters: [{ ...meter(0, 7, 8), denominator: 3 }] },
      { ...data, meters: [meter(0, 0, 4)] },
      { ...data, meters: [meter(0, 2.5, 4)] },
      { ...data, sections: [{ id: createId(), step: 0, name: "  " }] },
      {
        ...data,
        sections: [{ id: data.tempos[0].id, step: 0, name: "Duplicate ID" }],
      },
