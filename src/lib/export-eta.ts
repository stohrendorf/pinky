...
export function createExportEta(now: () => number = () => performance.now()) {
  ...
  function update(stage: string, progress: number | null): number | null {
    ...
    speed =
      speed === null ? observedSpeed : speed + weight * (observedSpeed - speed);
    previous = { time, progress };
    if (
      time - first.time < WARMUP_MS ||
      progress - first.progress < MIN_PROGRESS ||
      speed <= 0
    ) {
      return null;
    }
...
export function formatStageEta(
  ...
  const roundedSeconds = Math.max(
    1,
    seconds < 10 ? Math.round(seconds) : Math.round(seconds / 5) * 5,
  );
