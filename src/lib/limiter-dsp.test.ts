...
function render(dsp: LimiterDSP, left: Float32Array, right = left, block = 128, meter = true) {
    ...
    for (let offset = 0; offset < l.length; offset += block) {
        dsp.process(left.subarray(offset, offset + block), right.subarray(offset, offset + block),
            l.subarray(offset, offset + block), r.subarray(offset, offset + block), meter);
    }
    return [l, r];
}

describe('sample-peak stereo-linked lookahead limiter DSP', () => {
    it.each([44100, 48000, 96000])('catches the first, isolated, block-boundary and final spikes at %i Hz', rate => {
    ...
    it('bypasses gain and drive exactly but keeps the same latency, including switches', () => {
        ...
        expect(render(dsp, new Float32Array(1000).fill(100))[0].every(x => Math.abs(x) <= 1)).toBe(true);
    });
    ...
    it('holds through the lookahead then releases monotonically at the configured rate', () => {
        ...
        input[0] = 10;
        const fast = new LimiterDSP(48000), slow = new LimiterDSP(48000);
    ...
    it('is independent of render block size and can disable metering entirely offline', () => {
        ...
        const first = new LimiterDSP(48000), second = new LimiterDSP(48000);
