...
import { idxOfNote, STEPS, transposePitch } from "./notes";
import { PROMO_CHAPTERS, PROMO_PART_OF, type PromoPart } from "./promo-demo";
...
export function promoScore(p: Project): PromoScore {
  ...
  for (const clip of p.arrangement) {
    ...
    for (const [instId, list] of Object.entries(pat.tracks)) {
      ...
      for (const note of list) {
        const pitch = clip.transpose
          ? transposePitch(note.pitch, clip.transpose)
          : note.pitch;
        if (!pitch) {
          continue;
        }
  ...
  notes.sort(
    (a, b) => a.t - b.t || a.inst.localeCompare(b.inst) || a.midi - b.midi,
  );
  ...
  const at = (inst: PromoPart) =>
    new Set(notes.filter((n) => n.inst === inst).map((n) => n.t));
  const impacts = at("impact"),
    claps = at("clap");
  ...
  const sections = Object.fromEntries(
    Object.entries(PROMO_CHAPTERS).map(([name, [from, to]]) => [
      name,
      [from * step, to * step] as [number, number],
    ]),
  );
  return {
    bpm: p.bpm,
    length: lengthSteps * step,
    notes,
    events,
    sections,
    instruments,
