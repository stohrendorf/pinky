...
import { DEFAULT_PARAMS, ensurePartials, genPartials } from "./instruments";
...
const harmonic = (count: number, falloff: number): PartialSpec[] =>
  genPartials({
    shape: "Harmonic",
    count,
    falloff,
...
const CHOIR: PartialSpec[] = [1, 0.5, 0.33, 0.24, 0.18, 0.13].map(
  (level, index) => ({
    ratio: index + 1,
    level,
  }),
);
const SOPRANO: PartialSpec[] = [1, 0.6, 0.42, 0.3, 0.22, 0.16, 0.12, 0.09].map(
  (level, index) => ({
    ratio: index + 1,
    level,
  }),
...
const up = (pitch: string, octaves: number): string =>
  `${pitch.slice(0, -1)}${Number(pitch.at(-1)) + octaves}`;
...
export const PROMO_CHAPTERS: Record<string, [number, number]> = {
  ...
  carve: [2 * BAR, 4 * BAR],
  title: [4 * BAR, 6 * BAR],
  features: [6 * BAR, 14 * BAR],
  break: [14 * BAR, 16 * BAR],
  climax: [16 * BAR, 19 * BAR],
  outro: [19 * BAR, PROMO_LENGTH_STEPS],
};
...
function compose(): Score {
  ...
  progression.forEach((ch, i) => {
    ...
    ARPS[ch].forEach((pitch, k) =>
      s.n("pluck", pitch, bar, k * 2, 1.5, k % 2 === 0 ? 1 : 0.8),
    );
  ...
  (["Dm", "Bb", "F"] as ChordName[]).forEach((ch, i) => {
    ...
    for (let st = 0; st < 16; st++) {
      s.n(
        ...
        st,
        1,
        0.45 + (st % 4 === 0 ? 0.4 : 0) + (st % 2 === 0 ? 0.15 : 0),
      );
    }
...
const LANES: {
  ...
  },
  {
    ...
    label: "Drums",
    color: "#ff9f43",
    parts: ["kick", "snare", "clap", "hat", "ohat"],
  },
  {
    name: "03 Low End",
    label: "Low End",
...
const FULL = 0.34;
const dB = (base: number, delta: number): number =>
  Math.round(base * 10 ** (delta / 20) * 1000) / 1000;
...
export function buildPromoDemo(): Project {
  ...
  const arrangement = SECTIONS.flatMap((section) =>
    LANES.flatMap((lane, track) => {
      const notes = score.tracks.get(section) || {};
      const tracks = Object.fromEntries(
        lane.parts
          .filter((part) => notes[part]?.length)
          .map((part) => [PROMO_ID[part], notes[part]!]),
      );
  ...
  return {
    ...
    automation: [
      ...
      // where the master limiter catches the climax at its -1 dBFS ceiling.
      {
        ...
        param: "vol",
        points: [
          { step: 0, value: dB(FULL, -11), curve: "hold" },
          { step: 32, value: dB(FULL, -6), curve: "ease-in" },
          { step: 63, value: dB(FULL, -3), curve: "hold" },
          { step: 64, value: FULL },
          { step: 224, value: FULL, curve: "ease-out" },
          { step: 226, value: dB(FULL, -3), curve: "hold" },
          { step: 255, value: dB(FULL, -3), curve: "hold" },
          { step: 256, value: FULL },
        ],
      },
      // the video's reverb: a 28 % wet mix of the engine's 2.2 s burst
      {
        ...
        target: "master",
        param: "rev",
        points: [{ step: 0, value: 0.74, curve: "hold" }],
      },
    ],
