...
const groove = (id: string, name: string, chordName: keyof typeof CHORDS, root: string, kickTurn = false, lead: NoteSpec[] = [], strings = false): Pattern => {
    ...
    const snare = [4, 12].map(start => ['D3', start, 1, start === 12 ? 1 : 0.9] as NoteSpec);
    const kicks: NoteSpec[] = kickTurn
        ? [['A1', 0, 1, 1], ['A1', 8, 1, 1], ['A1', 10, 1, 0.85]]
        : [['A1', 0, 1, 1], ['A1', 6, 1, 0.85], ['A1', 8, 1, 1]];
...
export function buildPromoDemo(): Project {
    const instruments = [
        ...
        instrument(ID.sub, 'Bass/Sub', color.sub, {q: 30, harm: 2, falloff: 0.5, att: 1.2, dec: 0.3, sus: 1, rel: 0.6, gain: 0.45}),
        ...
        instrument(ID.bass, 'Bass/Pulse', color.bass, {q: 25, harm: 3, falloff: 0.5, att: 0.005, dec: 0.25, sus: 0.6, rel: 0.15, gain: 0.45}),
    ...
    const sourcePatterns = [
        ...
        pattern('1c02ba01-71ab-435d-9337-1450f2571002', '02 Arrival/spectrum carve', {
            ...
            [ID.boom]: notes(['G1', 0, 2, 1], ['G1', 16, 2, 0.9], ['G1', 26, 2, 0.7], ['G1', 29, 2, 0.8])
        }),
        pattern('1c02ba01-71ab-435d-9337-1450f2571003', '03 Title/pinky hit', {
            [ID.impact]: notes(['G1', 0, 4, 1]), [ID.boom]: notes(['D1', 0, 3, 1]),
            ...
            [ID.kick]: notes(['A1', 24, 1, 0.7], ['A1', 30, 1, 0.8])
        }),
        groove('1c02ba01-71ab-435d-9337-1450f2571004', '04 Groove/d minor pulse', 'dm', 'D2'),
        groove('1c02ba01-71ab-435d-9337-1450f2571005', '05 Groove/d minor turn', 'dm', 'D2', true),
        groove('1c02ba01-71ab-435d-9337-1450f2571006', '06 Groove/b flat pulse', 'bb', 'A#1'),
        groove('1c02ba01-71ab-435d-9337-1450f2571007', '07 Groove/b flat turn', 'bb', 'A#1', true),
        groove('1c02ba01-71ab-435d-9337-1450f2571008', '08 Groove/f major signal', 'f', 'F2', false, [
            ['D5', 0, 3, 0.9], ['F5', 4, 2, 0.9], ['E5', 6, 2, 0.9], ['D5', 8, 6, 0.9]
        ], true),
        groove('1c02ba01-71ab-435d-9337-1450f2571009', '09 Groove/f major turn', 'f', 'F2', true, [
            ['C5', 0, 4, 0.9], ['A4', 4, 3, 0.9], ['D5', 8, 8, 0.9]
        ], true),
        groove('1c02ba01-71ab-435d-9337-1450f2571010', '10 Groove/c major answer', 'c', 'C2', false, [
            ['F5', 0, 3, 0.9], ['G5', 4, 2, 0.9], ['A5', 6, 2, 0.9], ['G5', 8, 6, 0.9]
        ], true),
        groove('1c02ba01-71ab-435d-9337-1450f2571011', '11 Groove/c major turn', 'c', 'C2', true, [
            ['F5', 0, 4, 0.9], ['E5', 4, 3, 0.9], ['D5', 8, 8, 0.9]
        ], true),
        pattern('1c02ba01-71ab-435d-9337-1450f2571012', '12 Break/floor drops', {
            [ID.boom]: notes(['G1', 0, 2, 1]), [ID.choir]: notes(...chord(CHORDS.dm, 0, 32, 1)),
            ...
            [ID.snare]: notes(...[0, 2, 4, 6, 8, 9, 10, 11, 12, 13, 14, 15].map((start, index) => ['D3', 16 + start, 1, 0.55 + index * 0.04] as NoteSpec))
        }),
        pattern('1c02ba01-71ab-435d-9337-1450f2571013', '13 Climax/d minor lift', {
            ...
            [ID.bass]: notes(...repeated(['D2'], [0, 2, 4, 6, 8, 10, 12, 14], 1.5, step => step % 4 === 0 ? 1 : 0.8)),
            [ID.pluck]: notes(...ARPS.dm.flatMap((pitch, index) => [[pitch, index * 2, 1.5, 0.9] as NoteSpec, [pitch, index * 2 + 1, 1, 0.5] as NoteSpec])),
            [ID.kick]: notes(...[0, 4, 8, 12, 14].map(step => ['A1', step, 1, step === 14 ? 0.8 : 1] as NoteSpec)),
            [ID.snare]: notes(['D3', 4, 1, 1], ['D3', 12, 1, 1]), [ID.clap]: notes(['D3', 4, 1, 0.8], ['D3', 12, 1, 0.8]),
        ...
        pattern('1c02ba01-71ab-435d-9337-1450f2571014', '14 Climax/b flat rise', {
            ...
            [ID.bass]: notes(...repeated(['A#1'], [0, 2, 4, 6, 8, 10, 12, 14], 1.5, step => step % 4 === 0 ? 1 : 0.8)),
        ...
        pattern('1c02ba01-71ab-435d-9337-1450f2571015', '15 Climax/f major release', {
            ...
            [ID.bass]: notes(...repeated(['F2'], [0, 2, 4, 6, 8, 10, 12, 14], 1.5, step => step % 4 === 0 ? 1 : 0.8)),
        ...
        pattern('1c02ba01-71ab-435d-9337-1450f2571016', '16 Outro/final hit', {
            [ID.impact]: notes(['G1', 0, 4, 1]), [ID.boom]: notes(['D1', 0, 3, 1]), [ID.bell]: notes(['D4', 0, 6, 0.8], ['A4', 0, 6, 0.5]),
