...
describe('Pinky Promo demo', () => {
    ...
    it('bundles the trailer soundtrack in the demo library', () => {
        ...
        expect(Math.max(...promo.arrangement.map(clip => clip.start + clip.len))).toBe(
            PROMO_LENGTH_STEPS,
        );
    ...
    it('raises the same picture cues the Python score did', () => {
        ...
        expect(
            Object.fromEntries(
                Object.entries(score.sections).map(([k, [a, b]]) => [k, [ms(a), ms(b)]]),
            ),
        ).toEqual({
            noise: [0, 4.8],
            carve: [4.8, 9.6],
    ...
    it('rides the master the way the trailer did: whisper, swell, hit, break, climax', () => {
        ...
        expect(promo.automation?.find(lane => lane.param === 'rev')?.points[0].value).toBeCloseTo(
            0.74,
            6,
        );
    });
