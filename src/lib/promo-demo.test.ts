...
import {
    buildPromoDemo, PROMO_CHAPTERS, PROMO_ID, PROMO_LENGTH_STEPS, type PromoPart
} from './promo-demo';
import {
    promoScore
} from './promo-score';
...
const PYTHON_NOTE_COUNTS: Record<PromoPart, number> = {
    air: 1, bass: 72, bell: 4, boom: 7, clap: 22, hat: 128, impact: 2, kick: 41, lead: 16, ohat: 11,
    pad: 60, pluck: 112, riser: 2, snare: 34, strings: 32, sub: 4, voice: 11
};
...
describe('Pinky Promo demo', () => {
    ...
    it('bundles the trailer soundtrack in the demo library', () => {
        ...
        expect(Math.max(...promo.arrangement.map(clip => clip.start + clip.len))).toBe(PROMO_LENGTH_STEPS);
    ...
    it('reproduces the Python score: every note of every part', () => {
        ...
        expect(of('bass').slice(0, 4).map(row))
            .toEqual([[14.4, 38, 0.3, 1], [14.85, 38, 0.225, 0.75], [15.3, 38, 0.225, 0.75], [15.6, 38, 0.3, 1]]);
    ...
    it('raises the same picture cues the Python score did', () => {
        ...
        expect(score.events.filter(e => e.kind === 'snare' && e.t >= 36 && e.t < 38.4)).toHaveLength(0);
        expect(Object.fromEntries(Object.entries(score.sections).map(([k, [a, b]]) => [k, [ms(a), ms(b)]]))).toEqual({
            noise: [0, 4.8], carve: [4.8, 9.6], title: [9.6, 14.4], features: [14.4, 33.6],
            break: [33.6, 38.4], climax: [38.4, 45.6], outro: [45.6, 50.4]
        });
    ...
    it('rides the master the way the trailer did: whisper, swell, hit, break, climax', () => {
        const vol = promo.automation?.find(lane => lane.target === 'master' && lane.param === 'vol');
        const at = (step: number) => vol!.points.find(p => p.step === step)!.value;
        const full = at(64);
