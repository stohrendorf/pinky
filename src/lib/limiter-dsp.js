...
export class LimiterDSP {
  ...
  configure({
    ...
    this.ceiling =
      Math.pow(10, bounded(ceilingDb, -12, 0, -1) / 20) * (1 - 1e-7);
    this.release = Math.exp(
      -1 / (bounded(release, 0.02, 1, 0.1) * this.sampleRate),
    );
  }
  ...
  process(inputLeft, inputRight, outputLeft, outputRight, meter = true) {
    ...
    for (let i = 0; i < outputLeft.length; i++) {
      ...
      while (
        this.head !== this.tail &&
        this.times[this.head] < n - this.latencyFrames
      ) {
      ...
      outputRight[i] = outR;
      if (meter) {
        this.meterPeak[0] = Math.max(
          this.meterPeak[0],
          Math.abs(outputLeft[i]),
        );
        this.meterPeak[1] = Math.max(
          this.meterPeak[1],
          Math.abs(outputRight[i]),
        );
    ...
    if (meter) {
      this.meterReduction = Math.max(
        this.meterReduction,
        -20 * Math.log10(minimumGain),
      );
    }
  }
