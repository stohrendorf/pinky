...
describe('Vivaldi Winter demo', () => {
    ...
    it('puts every note of every part into the arrangement at its pitch', () => {
        ...
        const total = (voice: string) =>
            performed.reduce(
                (sum, notes) => sum + notes.filter(note => note.voice === voice).length,
                0,
            );
        expect(notesOf(WINTER_ID.solo).length).toBe(total('solo'));
        expect(total('solo')).toBeGreaterThan(1027 + 151 + 982);
        expect(notesOf(WINTER_ID.vn1).length + notesOf(WINTER_ID.pizz1).length).toBe(
            649 + 273 + 532,
        );
        expect(notesOf(WINTER_ID.vn2).length + notesOf(WINTER_ID.pizz2).length).toBe(
            633 + 273 + 508,
        );
        ...
        expect(
            notesOf(WINTER_ID.violone).every(note => idxOfNote[note.pitch] >= idxOfNote['C1']),
        ).toBe(true);
    ...
    it('performs what the print says beyond the notes', () => {
        ...
        const bar4 = first.filter(
            note => note.voice === 'solo' && note.start >= 3 * 32 && note.start < 4 * 32,
        );
        ...
        expect(second.filter(note => note.voice === 'vla').every(note => note.vel <= 0.4)).toBe(
            true,
        );
        ...
        const bass = new Set(
            first.filter(note => note.voice === 'vc').map(note => `${note.start}/${note.midi}`),
        );
        const chords = first.filter(
            note => note.voice === 'hpsd' && !bass.has(`${note.start}/${note.midi}`),
        );
        expect(chords.length).toBeGreaterThan(200);
        for (const chord of chords) {
            expect(
                first.some(note => note.voice === 'vn1' && note.start === chord.start),
                `chord at ${chord.start}`,
            ).toBe(true);
            expect(
                first.some(note => note.voice === 'vla' && note.start === chord.start),
                `chord at ${chord.start}`,
            ).toBe(true);
        }
        ...
        for (const [name, steps] of [
            ...
            const chord = demo.patterns.filter(
                pattern => pattern.name.includes(name) && !pattern.name.includes('Venti'),
            );
    ...
    it('lets the winds of the sonnet blow under the bars that paint them', () => {
        ...
        const barsOf = (movement: number, voice: string) =>
            performMovement(WINTER_MOVEMENTS[movement])
                ...
                .map(note => [
                    Math.floor(note.start / WINTER_MOVEMENTS[movement].data.stepsPerBar) + 1,
                    Math.ceil(
                        (note.start + note.len) / WINTER_MOVEMENTS[movement].data.stepsPerBar,
                    ),
                ]);
        const inside = (ranges: number[][], windows: [number, number][]) =>
            ranges.every(([from, to]) => windows.some(([a, b]) => from >= a && to <= b));
        ...
        const pan = (id: string) =>
            demo.automation!.find(lane => lane.target === id && lane.param === 'pan')!;
    ...
    it('makes the soloist sing in the Largo: vocal formants, a moving vowel and portamento', () => {
        ...
        const laneOf = (param: string) =>
            demo.automation!.find(lane => lane.target === WINTER_ID.solo && lane.param === param)!;
        ...
        const linked = (movement: number) =>
            demo.arrangement
                .filter(clip =>
                    placeSections().some(
                        section => section.movement === movement && section.start === clip.start,
                    ),
                )
                .flatMap(clip =>
                    (
                        demo.patterns.find(candidate => candidate.id === clip.patternId)!.tracks[
                            WINTER_ID.solo
                        ] || []
                    )
        ...
        for (const { note, clip } of slides) {
            const part = demo.patterns.find(candidate => candidate.id === clip.patternId)!.tracks[
                WINTER_ID.solo
            ];
            ...
            expect(Math.abs(idxOfNote[target!.pitch] - idxOfNote[note.pitch])).toBeLessThanOrEqual(
                5,
            );
    ...
    it("keeps the automation inside the song and the master under the engine's headroom", () => {
        for (const lane of demo.automation!) {
            ...
            expect(
                lane.points.every((point, i) => !i || point.step >= lane.points[i - 1].step),
                lane.param,
            ).toBe(true);
            expect(
