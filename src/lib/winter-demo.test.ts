...
import { idxOfNote, noteByName } from "./notes";
import { buildDemoProject } from "./project";
import { isPitched, measureNote } from "./timbre-analysis";
import {
  decodeWinterPart,
  performMovement,
  placeSections,
...
import { WINTER_SCORE } from "./winter-score";

const demo = buildDemoProject("winter");
const instrument = (name: string) =>
  demo.instruments.find((candidate) => candidate.name === name)!;
const notesOf = (id: string) =>
  demo.arrangement.flatMap((clip) => {
    const pattern = demo.patterns.find(
      (candidate) => candidate.id === clip.patternId,
    )!;
    return (pattern.tracks[id] || []).map((note) => ({
      ...note,
      at: clip.start + note.start,
    }));
  });
const songEnd = Math.max(
  ...demo.arrangement.map((clip) => clip.start + clip.len),
);
...
describe("Vivaldi Winter demo", () => {
  it("carries every note of the edition, movement by movement", () => {
    WINTER_SCORE.forEach((movement, index) => {
      ...
      for (const [part, expected] of Object.entries(edition.notes)) {
        const notes = decodeWinterPart(
          movement.parts[part as keyof typeof movement.parts],
        );
        expect(notes.length, `${movement.title} ${part}`).toBe(expected);
        // in playing order, every note inside the movement
        expect(
          notes.every((note, i) => !i || note.start >= notes[i - 1].start),
        ).toBe(true);
        ...
        expect(end, `${movement.title} ${part}`).toBeLessThanOrEqual(
          movement.bars * movement.stepsPerBar,
        );
        expect(end, `${movement.title} ${part}`).toBeGreaterThan(
          (movement.bars - 1) * movement.stepsPerBar,
        );
    ...
    for (const [part, [low, high]] of Object.entries(RANGE)) {
      const midi = WINTER_SCORE.flatMap((movement) =>
        decodeWinterPart(movement.parts[part as keyof typeof movement.parts]),
      ).map((note) => note.midi);
  ...
  it("is the whole concerto: three movements in their own metres, with silences between them", () => {
    ...
    const [first, second, third] = [0, 1, 2].map((movement) =>
      sections.filter((section) => section.movement === movement),
    );
    ...
    expect(second.reduce((sum, section) => sum + section.span, 0)).toBe(
      18 * 64,
    );
    expect(second.reduce((sum, section) => sum + section.len, 0)).toBe(18 * 64);
    expect(third.reduce((sum, section) => sum + section.span, 0)).toBe(
      153 * 12,
    );
    ...
    expect(first.reduce((sum, section) => sum + section.len, 0)).toBe(
      63 * 32 + 16,
    );
    expect(second[0].start - (first.at(-1)!.start + first.at(-1)!.len)).toBe(
      24,
    );
    expect(third[0].start - (second.at(-1)!.start + second.at(-1)!.len)).toBe(
      24,
    );
    ...
    for (const movement of [first, second, third]) {
      movement
        .slice(1)
        .forEach((section, i) =>
          expect(section.start).toBe(movement[i].start + movement[i].len),
        );
      movement
        .slice(1)
        .forEach((section, i) =>
          expect(section.from).toBe(movement[i].from + movement[i].span),
        );
    }
  ...
  it("resolves every clip, track and lane, and never stacks clips on one lane", () => {
    ...
    expect(
      demo.arrangement.every((clip) =>
        demo.patterns.some((pattern) => pattern.id === clip.patternId),
      ),
    ).toBe(true);
    expect(
      demo.arrangement.every(
        (clip) => clip.track >= 0 && clip.track < demo.tracks.length,
      ),
    ).toBe(true);
    expect(
      demo.automation!.every(
        (candidate) =>
          candidate.target === "master" ||
          demo.instruments.some(({ id }) => id === candidate.target),
      ),
    ).toBe(true);
    expect(demo.automationOrder).toEqual(
      demo.automation!.map((candidate) => candidate.id),
    );
    expect(
      new Set(
        [
      ...
      demo.instruments.length +
        demo.patterns.length +
        demo.arrangement.length +
        demo.automation!.length,
    );
    for (const clip of demo.arrangement) {
      ...
      const overlapping = demo.arrangement.filter(
        (other) =>
          other !== clip &&
          other.track === clip.track &&
          other.start < clip.start + clip.len &&
          clip.start < other.start + other.len,
      );
      ...
      for (const notes of Object.values(pattern.tracks)) {
        expect(
          notes.every(
            (note) =>
              note.start >= 0 &&
              note.start + note.len <= pattern.steps &&
              noteByName[note.pitch],
          ),
        ).toBe(true);
      }
    }
  ...
  it("puts every note of every part into the arrangement at its pitch", () => {
    ...
    const total = (voice: string) =>
      performed.reduce(
        (sum, notes) =>
          sum + notes.filter((note) => note.voice === voice).length,
        0,
      );
    expect(notesOf(WINTER_ID.solo).length).toBe(total("solo"));
    expect(total("solo")).toBeGreaterThan(1027 + 151 + 982);
    expect(
      notesOf(WINTER_ID.vn1).length + notesOf(WINTER_ID.pizz1).length,
    ).toBe(649 + 273 + 532);
    expect(
      notesOf(WINTER_ID.vn2).length + notesOf(WINTER_ID.pizz2).length,
    ).toBe(633 + 273 + 508);
    ...
    expect(
      notesOf(WINTER_ID.violone).every(
        (note) => idxOfNote[note.pitch] >= idxOfNote["C1"],
      ),
    ).toBe(true);
    ...
    expect(
      notesOf(WINTER_ID.violone)
        .filter((note) => note.at === 0)
        .map((note) => note.pitch),
    ).toEqual(["F3"]);
    // the ranges land in the DAW's registers (notes.ts: middle C = C5)
    const solo = notesOf(WINTER_ID.solo).map(
      (note) => noteByName[note.pitch].freq,
    );
  ...
  it("performs what the print says beyond the notes", () => {
    ...
    const bar4 = first.filter(
      (note) =>
        note.voice === "solo" && note.start >= 3 * 32 && note.start < 4 * 32,
    );
    ...
    const bass = new Set(
      first
        .filter((note) => note.voice === "vc")
        .map((note) => `${note.start}/${note.midi}`),
    );
    const chords = first.filter(
      (note) =>
        note.voice === "hpsd" && !bass.has(`${note.start}/${note.midi}`),
    );
    expect(chords.length).toBeGreaterThan(200);
    for (const chord of chords) {
      expect(
        first.some(
          (note) => note.voice === "vn1" && note.start === chord.start,
        ),
        `chord at ${chord.start}`,
      ).toBe(true);
      expect(
        first.some(
          (note) => note.voice === "vla" && note.start === chord.start,
        ),
        `chord at ${chord.start}`,
      ).toBe(true);
    }
    // the Lento is stretched: its clips are twice their bar count
    const lento = demo.patterns.filter((pattern) =>
      pattern.name.includes("Lento"),
    );
    ...
    for (const [name, steps] of [
      ...
      const chord = demo.patterns.filter(
        (pattern) =>
          pattern.name.includes(name) && !pattern.name.includes("Venti"),
      );
  ...
  it("lets the winds of the sonnet blow under the bars that paint them", () => {
    const borea = instrument("Orchestra/Venti/Borea"),
      sirocco = instrument("Orchestra/Venti/Sirocco");
    ...
    expect(borea.params.noiseFreq).toBeGreaterThan(
      sirocco.params.noiseFreq * 4,
    );
    ...
    const barsOf = (movement: number, voice: string) =>
      performMovement(WINTER_MOVEMENTS[movement])
        .filter((note) => note.voice === voice)
        .map((note) => [
          Math.floor(note.start / WINTER_MOVEMENTS[movement].data.stepsPerBar) +
            1,
          Math.ceil(
            (note.start + note.len) /
              WINTER_MOVEMENTS[movement].data.stepsPerBar,
          ),
        ]);
    const inside = (ranges: number[][], windows: [number, number][]) =>
      ranges.every(([from, to]) =>
        windows.some(([a, b]) => from >= a && to <= b),
      );
    expect(barsOf(0, "borea").length).toBeGreaterThan(5);
    expect(
      inside(barsOf(0, "borea"), [
        [12, 18],
        [27, 38],
        [47, 55],
      ]),
    ...
    const velocities = performMovement(WINTER_MOVEMENTS[2])
      .filter((note) => note.voice === "borea")
      .map((note) => note.vel);
    ...
    expect(venti).toBe(6);
    for (const clip of demo.arrangement.filter(
      (candidate) => candidate.track === venti,
    )) {
      const pattern = demo.patterns.find(
        (candidate) => candidate.id === clip.patternId,
      )!;
      expect(
        Object.keys(pattern.tracks).every(
          (id) => id === WINTER_ID.borea || id === WINTER_ID.sirocco,
        ),
      ).toBe(true);
    }
    // "tutti i Venti in guerra": the two winds change sides through the last tutti
    const pan = (id: string) =>
      demo.automation!.find(
        (lane) => lane.target === id && lane.param === "pan",
      )!;
    const war = placeSections().find(
      (section) => section.name === "N tutti li venti",
    )!;
    expect(
      pan(WINTER_ID.borea)
        .points.filter((point) => point.step <= war.start)
        .at(-1)!.value,
    ).toBeLessThan(0);
    expect(pan(WINTER_ID.borea).points.at(-1)!.value).toBeGreaterThan(0.5);
    expect(
      pan(WINTER_ID.sirocco)
        .points.filter((point) => point.step <= war.start)
        .at(-1)!.value,
    ).toBeGreaterThan(0);
  ...
  it("makes the soloist sing in the Largo: vocal formants, a moving vowel and portamento", () => {
    ...
    const laneOf = (param: string) =>
      demo.automation!.find(
        (lane) => lane.target === WINTER_ID.solo && lane.param === param,
      )!;
    const [largo, lento, borea] = [
      "E la pioggia",
      "M il vento Sirocco (Lento)",
      "N il vento Borea",
    ].map((name) => placeSections().find((section) => section.name === name)!);
    ...
    const valueAt = (lane: typeof formant, step: number) =>
      lane.points.filter((point) => point.step <= step).at(-1)!.value;
    expect(valueAt(formant, 0)).toBe(0);
    expect(valueAt(formant, largo.start)).toBeGreaterThanOrEqual(0.3);
    expect(
      valueAt(formant, largo.start + largo.len * 2),
    ...
    const f2 = laneOf("f2");
    expect(
      new Set(f2.points.map((point) => point.value)).size,
    ).toBeGreaterThanOrEqual(3);
    expect(
      f2.points.every((point) => point.value >= 500 && point.value <= 3500),
    ).toBe(true);
    // the emphasis trim is given back
    expect(valueAt(laneOf("gain"), largo.start)).toBeGreaterThan(
      solo.params.gain,
    );
    ...
    const linked = (movement: number) =>
      demo.arrangement
        .filter((clip) =>
          placeSections().some(
            (section) =>
              section.movement === movement && section.start === clip.start,
          ),
        )
        .flatMap((clip) =>
          (
            demo.patterns.find((candidate) => candidate.id === clip.patternId)!
              .tracks[WINTER_ID.solo] || []
          )
            .filter((note) => note.legatoTo)
            .map((note) => ({ note, clip })),
        );
    ...
    for (const { note, clip } of slides) {
      const part = demo.patterns.find(
        (candidate) => candidate.id === clip.patternId,
      )!.tracks[WINTER_ID.solo];
      const target = part.find(
        (candidate) =>
          candidate.start === note.legatoTo!.start &&
          candidate.pitch === note.legatoTo!.pitch,
      );
      ...
      expect(
        Math.abs(idxOfNote[target!.pitch] - idxOfNote[note.pitch]),
    ...
    for (const id of [WINTER_ID.pizz1, WINTER_ID.pizz2]) {
      const pan = demo.automation!.find(
        (lane) => lane.target === id && lane.param === "pan",
      )!;
      expect(
        new Set(pan.points.map((point) => point.value)).size,
      ).toBeGreaterThanOrEqual(3);
      expect(pan.points.every((point) => Math.abs(point.value) <= 0.7)).toBe(
        true,
      );
    }
  });
  ...
  it("voices a period string band the other demos do not have", () => {
    ...
    expect(names).toEqual([
      ...
      "Orchestra/Strings/Violins I",
      "Orchestra/Strings/Violins II",
      "Orchestra/Strings/Violas",
      "Orchestra/Strings/Violoncelli",
      "Orchestra/Strings/Violone",
      "Orchestra/Strings/Violins I pizz.",
      "Orchestra/Strings/Violins II pizz.",
      "Orchestra/Continuo/Harpsichord",
      "Orchestra/Venti/Borea",
      "Orchestra/Venti/Sirocco",
    ]);
    for (const song of [
      "axelf",
      "toccata",
    ...
    expect(
      instrument("Orchestra/Strings/Solo Violin").params.vib,
    ).toBeGreaterThan(10);
    ...
    expect(instrument("Orchestra/Strings/Violins I").params.pan).toBeLessThan(
      -0.3,
    );
    expect(
      instrument("Orchestra/Strings/Violoncelli").params.pan,
    ).toBeGreaterThan(0.3);
  });
  ...
  it("renders every instrument as a pitch at the register and note length it plays", () => {
    for (const inst of demo.instruments) {
      ...
      const sorted = [...notes].sort(
        (a, b) => noteByName[a.pitch].freq - noteByName[b.pitch].freq,
      );
      const len = median(notes.map((note) => note.len));
      for (const pitch of [
        sorted[0].pitch,
        sorted[Math.floor(sorted.length / 2)].pitch,
        sorted.at(-1)!.pitch,
      ]) {
        const measured = measureNote(inst.params, pitch, len, demo.bpm);
        expect(
          measured.clarity,
          `${inst.name} on ${pitch} for ${len} steps`,
        ).toBeGreaterThanOrEqual(0.65);
        expect(measured.peakDb, `${inst.name} on ${pitch}`).toBeGreaterThan(
          -30,
        );
      }
    }
  ...
  it("keeps the automation inside the song and the master under the engine's headroom", () => {
    for (const lane of demo.automation!) {
      ...
      expect(
        lane.points.every(
          (point, i) => !i || point.step >= lane.points[i - 1].step,
        ),
        lane.param,
      ).toBe(true);
      expect(
        lane.points.every((point) => point.step >= 0 && point.step <= songEnd),
        lane.param,
      ).toBe(true);
    }
    const master = demo.automation!.find(
    ...
    expect(
      Math.max(...master.points.map((point) => point.value)),
    ).toBeLessThanOrEqual(0.3);
    const rev = demo.automation!.find(
      (lane) => lane.target === "master" && lane.param === "rev",
    )!;
    const largo = placeSections().find((section) => section.movement === 1)!;
    const wetAtLargo = rev.points
      .filter((point) => point.step <= largo.start)
      .at(-1)!.value;
