...
const instrumentTracks = (
  demo: ReturnType<typeof buildDemoProject>,
  instrumentName: string,
  patternName?: string,
) => {
  const instrument = demo.instruments.find(
    (candidate) => candidate.name === instrumentName,
  )!;
  const patterns = patternName
    ? demo.patterns.filter((pattern) => pattern.name === patternName)
    : demo.patterns;
...
const songLengthInSteps = (demo: ReturnType<typeof buildDemoProject>) =>
  Math.max(...demo.arrangement.map(({ start, len }) => start + len));

const sectionPatternIds = (demo: ReturnType<typeof buildDemoProject>) => {
  const starts = [...new Set(demo.arrangement.map((clip) => clip.start))].sort(
    (a, b) => a - b,
  );
...
describe("original genre demo songs", () => {
  it.each(ORIGINAL_DEMOS)(
    "loads $title as a complete playable project",
    ({ id, minInstruments, minPatterns }) => {
      ...
      expect(
        demo.patterns.some((pattern) =>
          Object.values(pattern.tracks).some((notes) => notes.length > 0),
        ),
      ).toBe(true);
      expect(demo.automation?.some((lane) => lane.points.length >= 2)).toBe(
        true,
      );
    },
  );
  ...
  it.each(ORIGINAL_DEMOS)(
    "gives $title a full-length, evolving arrangement",
    ({ id }) => {
      const demo = buildDemoProject(id);
      ...
      expect(
        new Set(demo.arrangement.map(({ patternId }) => patternId)).size,
  ...
  it("establishes the funk identity in the playable project data", () => {
    ...
    const rhythmGuitar = pocket.instruments.find(
      (instrument) => instrument.name === "Percussion/Shard Guitar",
    );

    expect(rhythmGuitar).toBeDefined();
    expect(
      pocket.patterns.some(
        (pattern) => (pattern.tracks[rhythmGuitar!.id]?.length ?? 0) >= 8,
      ),
    ).toBe(true);
  });
  ...
  it("ships Pocket Theory with a bass-forward palette that still reads as instruments", () => {
    const pocket = buildDemoProject("pocket");
    const bass = pocket.instruments.find(
      (instrument) => instrument.name === "Bass/Chrome Tongue",
    )!;
    const rhythmKeys = pocket.instruments.find(
      (instrument) => instrument.name === "Keys/Glass Circuit",
    )!;
    const pad = pocket.instruments.find(
      (instrument) => instrument.name === "Pads/Choir Voltage",
    )!;
    const lead = pocket.instruments.find(
      (instrument) => instrument.name === "Synth/Radio Animal",
    )!;
    const accents = pocket.instruments.find(
      (instrument) => instrument.name === "Synth/Neon Chordbite",
    )!;
    const shardGuitar = pocket.instruments.find(
      (instrument) => instrument.name === "Percussion/Shard Guitar",
    )!;
    const clap = pocket.instruments.find(
      (instrument) => instrument.name === "Drums/Hand Clap",
    )!;
    ...
    expect(
      instrumentTracks(pocket, "Bass/Chrome Tongue").every(
        (note) => noteByName[note.pitch].freq >= 45,
      ),
    ).toBe(true);
    expect(
      pocket.instruments.some(
        (instrument) => instrument.name === "Bass/Slap Attack",
      ),
    ).toBe(false);
    const voices = [bass, rhythmKeys, pad, lead, accents, shardGuitar];

    expect(
      rhythmKeys.params.partials!.some(
        (partial) => !Number.isInteger(partial.ratio),
      ),
    ).toBe(true);
    ...
    expect(
      new Set(voices.map((instrument) => instrument.params.q)).size,
    ).toBeGreaterThanOrEqual(4);
    expect(
      pocket.automation?.some(
        (lane) => lane.target === lead.id && lane.param === "gain",
      ),
    ).toBe(true);
    expect(
      pocket.automation?.some(
        (lane) => lane.target === pad.id && lane.param === "gain",
      ),
    ).toBe(true);
  ...
  it("adds shaped expression sparingly to Pocket Theory", () => {
    ...
    const notes = demo.patterns.flatMap((pattern) =>
      Object.values(pattern.tracks).flat(),
    );
    const links = notes.flatMap((note) =>
      note.legatoTo ? [note.legatoTo] : [],
    );
    const curves = (demo.automation ?? []).flatMap((lane) =>
      lane.points.map((point) => point.curve).filter(Boolean),
    );
  ...
  it("keeps its original music distinct from the requested reference works", () => {
    const names = ORIGINAL_DEMOS.map(({ id }) =>
      buildDemoProject(id as DemoSong).patterns.map((pattern) => pattern.name),
    );
  ...
  it("gives Bit Horizon a focused chiptune palette and a developed arcade-song arc", () => {
    ...
    const lead = instrumentTracks(
      demo,
      "Lead/Star Runner",
      "03 Flight/Star runner",
    );
    const theme = instrumentTracks(
      demo,
      "Lead/Star Runner",
      "04 Horizon/Second signal",
    );
    const breakdown = pattern(demo, "05 Drift/Console memory");
    const curves = demo.automation!.flatMap((lane) =>
      lane.points.map((point) => point.curve),
    );
    ...
    expect(
      instruments.some((params) => params.noise >= 0.9 && params.dec <= 0.08),
    ).toBe(true);
    expect(
      instruments.some((params) =>
        params.partials?.every(
          (partial) =>
            Number.isInteger(partial.ratio) && partial.ratio % 2 === 1,
        ),
      ),
    ).toBe(true);
    expect(
      instruments.some(
        (params) => params.pitchDrop >= 18 && params.pitchTime <= 0.08,
      ),
    ).toBe(true);
    expect(new Set(notes.map((note) => note.vel)).size).toBeGreaterThanOrEqual(
      6,
    );
    expect(lead.filter((note) => note.legatoTo).length).toBeGreaterThanOrEqual(
      2,
    );
    expect(theme.map((note) => note.pitch)).not.toEqual(
      lead.map((note) => note.pitch),
    );
    expect(Object.keys(breakdown.tracks).length).toBeGreaterThanOrEqual(2);
    expect(demo.patterns).toHaveLength(16);
    expect(
      new Set(sections.map((section) => section.join("|"))).size,
