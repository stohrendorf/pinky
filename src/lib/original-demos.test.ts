...
describe('original genre demo songs', () => {
    ...
    it('establishes the funk identity in the playable project data', () => {
        ...
        expect(
            pocket.patterns.some(pattern => (pattern.tracks[rhythmGuitar!.id]?.length ?? 0) >= 8),
        ).toBe(true);
    });
    ...
    it('ships Pocket Theory with a bass-forward palette that still reads as instruments', () => {
        ...
        expect(pocket.instruments.some(instrument => instrument.name === 'Bass/Slap Attack')).toBe(
            false,
        );
        const voices = [bass, rhythmKeys, pad, lead, accents, shardGuitar];

        expect(rhythmKeys.params.partials!.some(partial => !Number.isInteger(partial.ratio))).toBe(
            true,
        );
        ...
        expect(new Set(voices.map(instrument => instrument.params.q)).size).toBeGreaterThanOrEqual(
            4,
        );
        expect(
            pocket.automation?.some(lane => lane.target === lead.id && lane.param === 'gain'),
        ).toBe(true);
        expect(
            pocket.automation?.some(lane => lane.target === pad.id && lane.param === 'gain'),
        ).toBe(true);
        ...
        expect(
            instruments.some(params =>
                params.partials?.every(
                    partial => Number.isInteger(partial.ratio) && partial.ratio % 2 === 1,
                ),
            ),
        ).toBe(true);
        expect(instruments.some(params => params.pitchDrop >= 18 && params.pitchTime <= 0.08)).toBe(
            true,
        );
