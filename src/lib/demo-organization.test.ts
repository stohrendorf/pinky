...
const DEMOS: {
...
}[] = [
    {
        ...
        payloadHash: '161bff3f76c5e34f6da8a1aa60acceb8422d481b3bce59d7456d394c88098d3b',
        originalHash: '509bf316c8fa7bd5a6cf156bcc5df978a24c41c32a26f4a116db522e64bf2302',
    ...
    },
    {
        ...
        payloadHash: '346b73349bd98e33d7ff72a864834f57952e4a747746ec11e9dd0da21cdcba09',
        originalHash: '656eaba262df1a8b75098d0fd4ed07c84a7e97752cbc1b335c80af20ce53bbda',
        groups: { Drums: 3, Percussion: 2, Bass: 2, Keys: 5, Strings: 5 },
        roots: ['dim arpeggio', 'D major hit', 'drum fill', 'valkyrie tutti', 'bells toll'],
        families: {
            Drums: ['Drums/'],
    ...
    },
    {
        ...
        payloadHash: '20045f64ab66192d291112742f874dc08db7583b28a1733ff139a9c7eadd624e',
        originalHash: '203872305fa8da48b74118bb34c3a65acef8fb275bebe8db29c4c8c94d2de998',
    ...
    },
    {
        ...
        payloadHash: 'ae3b5b04879daaa22ff78c37281d72a94a8a9083fe64dbd94d2d62c1a0b298ce',
        originalHash: 'c14e79d235dc51e6e6f0b05581ffa7a40d60e7b05a4d0503df86c15af35f3b27',
        groups: { 'Lead Vocal': 10, Choir: 6, Strings: 8, Harp: 3, Drums: 3, FX: 2, Keys: 3 },
        roots: [
            'Hinge — Bass wakes',
            'Machine I — Groove',
            'Machine II — Groove',
...
describe('JSON demo organization', () => {
    ...
    it.each(DEMOS)('preserves the entire $song musical payload', ({ data, payloadHash }) => {
    ...
    it.each(DEMOS)('keeps $song folders nonempty, unique and below 20 children', ({ data }) => {
        const children = new Map<string, Set<string>>();
        expect(new Set(data.patterns.map(pattern => pattern.name)).size).toBe(data.patterns.length);
        for (const pattern of data.patterns) {
            const parts = pattern.name.split('/');
            expect(pattern.name).toMatch(/^\d{2} [^/]+(?:\/[^/]+){1,2}$/);
            expect(parts.every(part => part.trim().length > 0 && part === part.trim())).toBe(true);
        ...
        for (const [folder, entries] of children) {
            expect(entries.size, folder).toBeGreaterThanOrEqual(folder.includes('/') ? 2 : 1);
    ...
    it.each(DEMOS)(
        ...
        ({ data, groups, roots, families }) => {
            const main = data.patterns.filter(pattern => pattern.name.startsWith('02 Main/'));
            const grouped = main.filter(pattern => pattern.name.split('/').length === 3);
            ...
            for (const pattern of grouped) {
                ...
                const played = Object.entries(pattern.tracks).filter(([, notes]) => notes.length);
    ...
    it.each(DEMOS)(
        ...
        ({ data }) => {
            const instruments = new Set(data.instruments.map(instrument => instrument.id));
            ...
            for (const pattern of data.patterns) {
                expect(Object.keys(pattern.tracks).every(id => instruments.has(id))).toBe(true);
            }
            ...
            for (const lane of data.automation ?? []) {
                expect(lane.target === 'master' || instruments.has(lane.target)).toBe(true);
            }
            expect(
                data.automationOrder?.every(id => data.automation?.some(lane => lane.id === id)),
            ).toBe(true);
        },
    );
});
