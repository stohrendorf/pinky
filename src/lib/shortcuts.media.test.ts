...
describe('multimedia shortcuts', () => {
    it('maps the standard media keys to transport actions', () => {
        expect(shortcuts).toMatch(/e\.key === 'MediaPlayPause' \|\| e\.key === 'MediaPlay'/);
        expect(shortcuts).toContain('togglePlay(e.shiftKey);');
        expect(shortcuts).toMatch(/e\.key === 'MediaTrackNext'[\s\S]*playPattern\(\);/);
        expect(shortcuts).toMatch(/e\.key === 'MediaTrackPrevious'[\s\S]*seekSong\(0\);/);
        expect(shortcuts).toMatch(/e\.key === 'MediaStop'[\s\S]*stopTransport\(\);/);
    });
