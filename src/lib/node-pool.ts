...
export class NodePool {
    ...
    get size(): number {
        return this.gains.length + this.biquads.length + this.panners.length + this.cooling.length;
    }
    ...
    takeGain(gain: number, context: BaseAudioContext): GainNode {
        const node = this.enabled && context === this.context ? this.gains.pop() : undefined;
        if (node) {
    ...
    takePanner(pan: number, context: BaseAudioContext): StereoPannerNode {
        const node = this.enabled && context === this.context ? this.panners.pop() : undefined;
        if (node) {
