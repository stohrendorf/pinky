...
function curPattern(): Pattern | null {
  ...
  return (
    p.patterns.find((pt) => pt.id === get(selPatId)) || p.patterns[0] || null
  );
}
...
export function removeInvalidLegatoLinks(notes: Note[]): void {
  notes.forEach((source) => {
    ...
    const target = notes.find(
      (note) =>
        note.start === source.legatoTo?.start &&
        note.pitch === source.legatoTo.pitch,
    );
...
export function updateLegatoTargets(
  ...
  notes.forEach((note) => {
    ...
    const target = movedTargets.get(
      `${originalTarget.pitch}:${originalTarget.start}`,
    );
    if (!target) {
      return;
    }
...
export function transposeSelectedNotes(semis: number): void {
  ...
  if (
    Math.min(...rows) + semis < 0 ||
    Math.max(...rows) + semis > NOTES.length - 1
  ) {
    return;
  }
