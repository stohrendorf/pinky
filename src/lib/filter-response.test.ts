...
describe("filterMagnitude", () => {
  it("models a stronger response at the fundamental than away from its resonant bands", () => {
    ...
    expect(filterMagnitude(params, "C4", 261.63)).toBeGreaterThan(
      filterMagnitude(params, "C4", 1000),
    );
  });
  ...
  it("keeps a low-note filter response stable across octaves", () => {
    ...
    const c0 =
      20 * Math.log10(filterMagnitude(params, "C0", noteByName.C0.freq));
    const c2 =
      20 * Math.log10(filterMagnitude(params, "C2", noteByName.C2.freq));
  ...
  it("includes the exact detuned centers of every unison rank", () => {
    ...
    expect(curve.map((point) => point.frequency)).toEqual(
      expect.arrayContaining([lowerRank, upperRank]),
    );
  });
  ...
  it("adds detuned unison ranks as stereo spectrum power", () => {
    ...
    const rankResponse = (cents: number) => {
      ...
      return {
        real: (nr * dr + ni * di) / den - 1,
        imaginary: (ni * dr - nr * di) / den,
      };
    };
    const lower = rankResponse(-40);
    const upper = rankResponse(40);
    const expected = Math.sqrt(
      (lower.real * lower.real +
        lower.imaginary * lower.imaginary +
        upper.real * upper.real +
        upper.imaginary * upper.imaginary) /
        2,
    );
