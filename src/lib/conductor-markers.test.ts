...
import { markerPoints, moveMarkerAt, removeMarkerAt, updateMarkerAt } from './conductor-markers';
...
describe('updateMarkerAt', () => {
    ...
    it('persists and re-edits a generic title for all-blank fields, preserving an existing title ID', () => {
        const generic = checked(
            updateMarkerAt(frozen(empty()), 16, draft({ name: ' \t', signature: '\n ' })),
        );
    ...
    it('extends any partial group while retaining its existing per-kind ID', () => {
        ...
        for (const kind of kinds) {
            ...
            const result = checked(
                updateMarkerAt(data, 16, draft({ name: 'Full', bpm: 140, signature: '3/8' })),
            );
    ...
    it('rejects invalid tempos without changing any part of the group', () => {
        ...
        for (const bpm of [0, 29.99, 300.01, NaN, Infinity, -Infinity]) {
            expect(() =>
                updateMarkerAt(data, 16, draft({ name: 'Changed', bpm, signature: '4/4' })),
    ...
    it('rejects overlong titles without modifying existing data', () => {
        const data = frozen(group());
        const before = structuredClone(data);
        expect(() => updateMarkerAt(data, 16, draft({ name: ` ${'x'.repeat(81)} ` }))).toThrow(
            new Error('Title must be at most 80 characters.'),
        );
        expect(data).toEqual(before);
    });
    ...
    it('validates the curve even when no tempo is requested', () => {
        ...
        for (const bpm of [undefined, 120]) {
            expect(() =>
                updateMarkerAt(data, 16, draft({ bpm, curve: 'smooth' as MarkerDraft['curve'] })),
    ...
    it('retries UUID collisions across all existing kinds and IDs allocated during this edit', () => {
        ...
        const result = checked(
            updateMarkerAt(data, 32, draft({ name: 'New', bpm: 120, signature: '7/8' })),
        );
...
describe('moveMarkerAt', () => {
    ...
    it.each([
        ...
        (kind, label) => {
            ...
            const data = frozen({
                ...all,
                [kind]: [all[kind][0], { ...all[kind][0], id: types.createId(), step: 32 }],
            });
            const before = structuredClone(data);
            expect(() => moveMarkerAt(data, 16, 32)).toThrow(
                new Error(`There is already a ${label} change here. Drop beside it instead.`),
            );
            expect(data).toEqual(before);
        },
    );
