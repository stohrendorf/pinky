...
const group = (step = 16): ConductorData => ({tempos: [tempo(step)], meters: [meter(step)], sections: [section(step)]});
...
const signatureError = 'Use a time signature such as 7/8 (1–32 beats; unit 1, 2, 4, 8 or 16).';
...
describe('updateMarkerAt', () => {
    ...
    it('accepts tempo bounds and fractional BPM with both curves', () => {
        for (const bpm of [30, 120.5, 300]) {
            for (const curve of ['hold', 'linear'] as const) {
                const result = checked(updateMarkerAt(frozen(empty()), 0, draft({bpm, curve})));
    ...
    it('parses whitespace, leading zeros, numerator bounds and every allowed signature unit', () => {
        for (const denominator of [1, 2, 4, 8, 16]) {
            for (const numerator of [1, 7, 32]) {
                const result = checked(updateMarkerAt(frozen(empty()), 0, draft({signature: ` \t0${numerator} \t/ 0${denominator}\n `})));
    ...
    it('trims titles before checking the 80-character limit and preserves interior spaces', () => {
        for (const name of ['  First  Verse \n', ` \t${'x'.repeat(80)}  `]) {
            const result = checked(updateMarkerAt(frozen(empty()), 0, draft({name})));
    ...
    it('persists and re-edits a generic title for all-blank fields, preserving an existing title ID', () => {
        const generic = checked(updateMarkerAt(frozen(empty()), 16, draft({name: ' \t', signature: '\n '})));
        ...
        expect(result).toEqual({...empty(), sections: [{...data.sections[0], name: 'Marker'}]});
    });

    it('omits a blank title whenever tempo or signature remains', () => {
        for (const fields of [{bpm: 120}, {signature: '7/8'}, {bpm: 120, signature: '7/8'}]) {
            const result = checked(updateMarkerAt(frozen(group()), 16, draft({name: ' \n ', ...fields})));
            expect(result.sections).toEqual([]);
    ...
    it('atomically edits every kind with stable IDs, fresh changed objects and untouched neighbors', () => {
        ...
        const fields = Object.freeze(draft({name: ' Bridge ', bpm: 90, curve: 'linear', signature: '5/16'}));
    ...
    it('removes unspecified kinds together without affecting entries at other steps', () => {
        ...
        for (const fields of [{name: 'Title'}, {bpm: 150}, {signature: '4/4'}]) {
            ...
            expect(result.meters.filter(marker => marker.step !== 16)).toEqual([data.meters[1]]);
            expect(result.sections.filter(marker => marker.step !== 16)).toEqual([data.sections[1]]);
            expect(result.tempos.some(marker => marker.step === 16)).toBe('bpm' in fields);
            expect(result.meters.some(marker => marker.step === 16)).toBe('signature' in fields);
            expect(result.sections.some(marker => marker.step === 16)).toBe('name' in fields);
        }
    });
    ...
    it('rejects invalid tempos without changing any part of the group', () => {
        ...
        for (const bpm of [0, 29.99, 300.01, NaN, Infinity, -Infinity]) {
            expect(() => updateMarkerAt(data, 16, draft({name: 'Changed', bpm, signature: '4/4'})))
    ...
    it('rejects overlong titles without modifying existing data', () => {
        const data = frozen(group());
        const before = structuredClone(data);
        expect(() => updateMarkerAt(data, 16, draft({name: ` ${'x'.repeat(81)} `})))
    ...
    it('validates the curve even when no tempo is requested', () => {
        ...
        for (const bpm of [undefined, 120]) {
            expect(() => updateMarkerAt(data, 16, draft({bpm, curve: 'smooth' as MarkerDraft['curve']})))
    ...
    it('retries UUID collisions across all existing kinds and IDs allocated during this edit', () => {
        ...
        const result = checked(updateMarkerAt(data, 32, draft({name: 'New', bpm: 120, signature: '7/8'})));
    ...
    it('does not reuse the ID of a kind removed in the same edit', () => {
        ...
        vi.spyOn(types, 'createId').mockReturnValueOnce(data.sections[0].id).mockReturnValueOnce(id);
    ...
    it.each(kinds)('rejects a 513th %s entry before creating IDs or applying other fields', kind => {
        const all = full();
    ...
    it('counts generic titles against the cap but not blank titles with timing', () => {
        ...
        expect(() => updateMarkerAt(data, 1000, draft())).toThrow(/at most 512 titles/);
...
describe('moveMarkerAt', () => {
    it('moves all kinds together in either direction with stable IDs, values and untouched neighbors', () => {
        ...
        for (const to of [0, 1_000_000]) {
            ...
            expect(markerPoints(result).some(point => point.step === 16)).toBe(false);
            for (const kind of kinds) {
                expect(result[kind]).not.toBe(data[kind]);
    ...
    it.each([
        ['tempos', 'tempo'], ['meters', 'time-signature'], ['sections', 'title']
    ] as const)('rejects a %s collision even for equal values without moving other kinds', (kind, label) => {
        const all = group();
        const data = frozen({...all, [kind]: [all[kind][0], {...all[kind][0], id: types.createId(), step: 32}]});
        const before = structuredClone(data);
        expect(() => moveMarkerAt(data, 16, 32))
            .toThrow(new Error(`There is already a ${label} change here. Drop beside it instead.`));
        expect(data).toEqual(before);
    });
    ...
    it('rejects a missing source even if the destination or same-step target exists only elsewhere', () => {
        for (const data of [frozen(empty()), frozen(group())]) {
            for (const to of [16, 32]) {
                expect(() => moveMarkerAt(data, 32, to)).toThrow(new Error('This marker no longer exists.'));
            }
        }
    });
