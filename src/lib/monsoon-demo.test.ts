...
describe('Bronze Monsoon demo', () => {
    ...
    it('sings the khoomei overtone melody as F2 automation over one held drone', () => {
        ...
        const rootAt = (step: number) =>
            transposed.some(clip => step >= clip.start && step < clip.start + clip.len)
                ? roots.G3
                : roots.D3;

        expect(
            part('Vocals/Khoomei Drone').every(note => note.pitch === 'D3' && note.len >= 28),
        ).toBe(true);
        ...
        expect(
            overtones.points.filter(point => rootAt(point.step) === roots.G3).length,
    ...
    it('lays the pokok under every kotekan on the jublag', () => {
        ...
        expect(
            part('Percussion/Tuned/Jublag', '04 Storm/pokok storm').map(note => note.pitch),
        ).toEqual(['D5', 'C5', 'D5', 'A5']);
    ...
    it('runs 24- and 12-step patterns as polymeter inside longer clips', () => {
        ...
        expect(
            part('Drums/Taiko/Odaiko', '04 Storm/taiko in three').map(note => note.start),
        ).toEqual([0, 6]);
        const [three] = hemiolaClips;
        expect(
            clipsOf('04 Storm/storm kotekan').filter(
                clip => clip.start >= three.start && clip.start < three.start + three.len,
            ).length,
    ...
    it('drives timbre with lanes that a mixer has no knob for', () => {
        ...
        expect(
            Math.max(...ombak.filter(point => point.step >= stormStart).map(point => point.value)),
        ).toBeGreaterThan(40);
    ...
    it('puts the middle section in 7/8 and lifts it a fourth', () => {
        ...
        expect(
            part('Drums/Taiko/Odaiko', '03 Seven Rains/taiko seven').map(note => note.start),
    ...
    it('shapes the whole arc with master and instrument automation', () => {
        ...
        expect(new Set(demo.automation!.map(candidate => candidate.param))).toEqual(
            new Set([
                'vol',
                'rev',
                'tilt',
        ...
        expect(
            lane('master', 'tilt').points.every(point => point.value >= -4 && point.value <= 2),
        ).toBe(true);
        expect(
            Math.max(...lane('FX/Monsoon Rain', 'gain').points.map(point => point.value)),
        ).toBeLessThanOrEqual(0.05);
        expect(curves).toEqual(new Set(['hold', 'linear', 'ease-in', 'ease-out', 'smooth']));
        expect(
            demo.automation!.every(candidate =>
                candidate.points.every(point => point.step >= 0 && point.step <= songEnd),
            ),
        ).toBe(true);
    });
});
