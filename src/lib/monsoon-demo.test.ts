...
const part = (name: string, patternName?: string) => {
  ...
  return demo.patterns
    .filter((pattern) => !patternName || pattern.name === patternName)
    .flatMap((pattern) => pattern.tracks[id] || []);
};
const lane = (targetName: string, param: string) => {
  const target = targetName === "master" ? "master" : instrument(targetName).id;
  return demo.automation!.find(
    (candidate) => candidate.target === target && candidate.param === param,
  )!;
};
const songEnd = Math.max(
  ...demo.arrangement.map((clip) => clip.start + clip.len),
);
const patternOf = (clip: { patternId: string }) =>
  demo.patterns.find((pattern) => pattern.id === clip.patternId)!;
const clipsOf = (patternName: string) =>
  demo.arrangement.filter((clip) => patternOf(clip).name === patternName);
...
describe("Bronze Monsoon demo", () => {
  ...
  it("never stacks two clips on one arranger lane", () => {
    for (const clip of demo.arrangement) {
      ...
      expect(
        overlapping,
        `${clip.patternId} on track ${clip.track} at ${clip.start}`,
      ).toEqual([]);
    }
  });
  ...
  it("uses a palette the other demos do not have", () => {
    ...
    expect(families).toEqual(
      expect.arrayContaining([
        "Drums/Taiko/Odaiko",
        "Drums/Kendang",
        "Percussion/Tuned/Gong Ageng",
        "Percussion/Tuned/Gendér Polos",
        "Percussion/Tuned/Handpan",
        "Winds/Shakuhachi",
        "Winds/Duduk",
        "Vocals/Khoomei Drone",
        "Strings/Kora",
        "Percussion/Tuned/Jublag",
        "Percussion/Tuned/Singing Bowl",
        "FX/Monsoon Wind",
        "FX/Storm Riser",
      ]),
    );
  ...
  it("interlocks the gendér kotekan into one continuous sixteenth line", () => {
    const polos = part(
      "Percussion/Tuned/Gendér Polos",
      "02 Bronze/kotekan one",
    );
    const sangsih = part(
      "Percussion/Tuned/Gendér Sangsih",
      "02 Bronze/kotekan one",
    );
    ...
    expect(
      sangsih.filter((note) => note.start % 2 === 1).length,
    ).toBeGreaterThanOrEqual(14);
    expect(steps.size).toBe(31);
    expect(instrument("Percussion/Tuned/Gendér Polos").params.pan).toBeLessThan(
      -0.4,
    );
    expect(
      instrument("Percussion/Tuned/Gendér Sangsih").params.pan,
    ).toBeGreaterThan(0.4);
    // bar modes, not a harmonic series
    expect(
      instrument("Percussion/Tuned/Gendér Polos").params.partials!.map(
  ...
  it("sings the khoomei overtone melody as F2 automation over one held drone", () => {
    ...
    const transposed = clipsOf("03 Seven Rains/khoomei seven").filter(
      (clip) => clip.transpose === 5,
    );
    const harmonicOf = (hz: number, root: number) => {
      const harmonic = hz / root;
      return Math.abs(harmonic - Math.round(harmonic)) < 0.01
        ? Math.round(harmonic)
        : null;
    };
    const rootAt = (step: number) =>
      transposed.some(
        (clip) => step >= clip.start && step < clip.start + clip.len,
      )
        ? roots.G3
        : roots.D3;

    expect(
      part("Vocals/Khoomei Drone").every(
        (note) => note.pitch === "D3" && note.len >= 28,
      ),
    ).toBe(true);
    ...
    expect(jaw.points.map((point) => point.step)).toEqual(
      overtones.points.map((point) => point.step),
    );
    expect(
      overtones.points.slice(0, -1).every((point) => point.curve === "hold"),
    ).toBe(true);
    expect(
      new Set(overtones.points.map((point) => point.value)).size,
    ).toBeGreaterThanOrEqual(8);
    expect(
      overtones.points.filter((point) => rootAt(point.step) === roots.G3)
        .length,
  ...
  it("lays the pokok under every kotekan on the jublag", () => {
    const stream = [
      ...part("Percussion/Tuned/Gendér Polos", "02 Bronze/kotekan one"),
      ...part("Percussion/Tuned/Gendér Sangsih", "02 Bronze/kotekan one"),
    ];
    const pokok = part("Percussion/Tuned/Jublag", "02 Bronze/pokok one");

    expect(pokok.map((note) => note.start)).toEqual([
    ...
    for (const note of pokok) {
      ...
      expect(
        noteByName[beat.pitch].freq / noteByName[note.pitch].freq,
      ).toBeCloseTo(2, 5);
    }
    expect(
      part("Percussion/Tuned/Jublag", "03 Seven Rains/pokok seven").every(
        (note) => note.len === 7,
      ),
    ).toBe(true);
    expect(
      part("Percussion/Tuned/Jublag", "04 Storm/pokok storm").map(
        (note) => note.pitch,
      ),
    ).toEqual(["D5", "C5", "D5", "A5"]);
    // the kotekan is doubled an octave down instead, when the storm is at its densest
    const doubled = clipsOf("04 Storm/storm kotekan").filter(
      (clip) => clip.transpose === -12,
    );
    expect(doubled.length).toBe(3);
    expect(new Set(doubled.map((clip) => clip.track)).size).toBe(1);
    expect(
      doubled.every((clip) =>
        clipsOf("04 Storm/storm kotekan").some(
          (other) => other.start === clip.start && !other.transpose,
        ),
      ),
    ).toBe(true);
    expect(
      clipsOf("04 Storm/pokok storm").some((clip) =>
        doubled.some((other) => other.start === clip.start),
      ),
    ).toBe(false);
  });

  it("runs 24- and 12-step patterns as polymeter inside longer clips", () => {
    const bowl = demo.patterns.find(
      (pattern) => pattern.name === "01 Rain/bowl drift",
    )!;
    const hemiola = demo.patterns.find(
      (pattern) => pattern.name === "04 Storm/taiko in three",
    )!;
    ...
    expect(
      part("Drums/Taiko/Odaiko", "04 Storm/taiko in three").map(
        (note) => note.start,
      ),
    ).toEqual([0, 6]);
    const [three] = hemiolaClips;
    expect(
      clipsOf("04 Storm/storm kotekan").filter(
        (clip) =>
          clip.start >= three.start && clip.start < three.start + three.len,
      ).length,
  ...
  it("bows the singing bowl and slides the sub through the storm ground as portamento chains", () => {
    ...
    expect(bowl[0].legatoTo).toEqual({
      pitch: "A5",
      start: 10,
      curve: "smooth",
    });
    expect(bowl[1].legatoTo).toEqual({
      pitch: "G5",
      start: 18,
      curve: "ease-out",
    });
    expect(
      instrument("Percussion/Tuned/Singing Bowl").params.att,
    ).toBeGreaterThanOrEqual(1);
    expect(
      instrument("Percussion/Tuned/Singing Bowl").params.voices,
    ).toBeGreaterThanOrEqual(3);
    expect(ground.map((note) => note.pitch)).toEqual(["A#2", "C3", "D3"]);
    expect(ground[0].legatoTo).toEqual({
      pitch: "C3",
      start: 8,
      curve: "ease-in",
    });
    expect(ground[1].legatoTo).toEqual({
      pitch: "D3",
      start: 16,
      curve: "ease-in",
    });
  ...
  it("drives timbre with lanes that a mixer has no knob for", () => {
    const values = (targetName: string, param: string) =>
      lane(targetName, param).points.map((point) => point.value);
    ...
    expect(
      Math.min(...values("Percussion/Tuned/Gendér Polos", "dec")),
    ).toBeLessThan(0.5);
    expect(
      values("Percussion/Tuned/Gendér Polos", "dec").at(-1),
    ).toBeGreaterThan(1.2);
    expect(lane("Percussion/Tuned/Gendér Sangsih", "dec").points).toEqual(
      lane("Percussion/Tuned/Gendér Polos", "dec").points,
    );
    ...
    expect(sangsihPan.map((point) => point.value)).toEqual(
      polosPan.map((point) => -point.value),
    );
    expect(Math.max(...polosPan.map((point) => point.value))).toBeGreaterThan(
      0.4,
    );
    ...
    expect(
      Math.max(
        ...ombak
          .filter((point) => point.step >= stormStart)
    ...
    expect(
      Math.max(...values("FX/Monsoon Wind", "noiseFreq")) /
        Math.min(...values("FX/Monsoon Wind", "noiseFreq")),
    ).toBeGreaterThan(4);
    ...
    expect(Math.max(...values("Winds/Shakuhachi", "noise"))).toBeGreaterThan(
      instrument("Winds/Shakuhachi").params.noise,
    );
    expect(Math.max(...values("Vocals/Khoomei Drone", "vib"))).toBeGreaterThan(
      instrument("Vocals/Khoomei Drone").params.vib,
    );
  ...
  it("puts the middle section in 7/8 and lifts it a fourth", () => {
    const seven = demo.patterns.filter((pattern) =>
      pattern.name.startsWith("03 Seven Rains/"),
    );
    const sevenClips = demo.arrangement.filter((clip) =>
      seven.some((pattern) => pattern.id === clip.patternId),
    );
    ...
    expect(
      sevenClips.filter((clip) => clip.transpose === 5).length,
    ).toBeGreaterThanOrEqual(12);
    expect(
      part("Drums/Taiko/Odaiko", "03 Seven Rains/taiko seven").map(
        (note) => note.start,
      ),
    ).toEqual([0, 6, 10, 14, 20, 24]);
  });

  it("opens the pentatonic In scale into D minor for the storm", () => {
    const pitchClasses = (names: string[]) =>
      new Set(names.map((pitch) => pitch.replace(/\d+$/, "")));
    ...
    const storm = pitchClasses(
      part("Strings/Kora", "04 Storm/storm kora").map((note) => note.pitch),
    );
    const ground = part("Bass/Bronze Sub", "04 Storm/storm ground").map(
      (note) => note.pitch,
    );
  ...
  it("slides the shakuhachi and duduk with shaped portamento only", () => {
    const links = demo.patterns
      .flatMap((pattern) => Object.values(pattern.tracks).flat())
      .flatMap((note) => (note.legatoTo ? [note] : []));

    expect(links.length).toBeGreaterThanOrEqual(12);
    expect(
      links.every(
        (note) =>
          note.legatoTo!.curve !== "linear" &&
          note.legatoTo!.start >= note.start + note.len,
      ),
    ).toBe(true);
  ...
  it("measures every pitched part as a clear pitch at a mix level that can be heard", () => {
    for (const { name, params, id } of demo.instruments) {
      ...
      const notes = demo.patterns.flatMap(
        (pattern) => pattern.tracks[id] || [],
      );
  ...
  it("shapes the whole arc with master and instrument automation", () => {
    const curves = new Set(
      demo.automation!.flatMap((candidate) =>
        candidate.points.map((point) => point.curve).filter(Boolean),
      ),
    );

    expect(demo.automation!.length).toBeGreaterThanOrEqual(20);
    expect(
      new Set(demo.automation!.map((candidate) => candidate.param)),
    ).toEqual(
      new Set([
        "vol",
        "rev",
    ...
    expect(
      lane("master", "tilt").points.every(
        (point) => point.value >= -4 && point.value <= 2,
      ),
    ).toBe(true);
    expect(
      Math.max(
        ...lane("FX/Monsoon Rain", "gain").points.map((point) => point.value),
      ),
    ).toBeLessThanOrEqual(0.05);
    expect(curves).toEqual(
      new Set(["hold", "linear", "ease-in", "ease-out", "smooth"]),
    );
    expect(
      demo.automation!.every((candidate) =>
        candidate.points.every(
          (point) => point.step >= 0 && point.step <= songEnd,
        ),
      ),
    ).toBe(true);
  });
