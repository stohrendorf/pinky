...
for (let oct = 0; oct < 10; oct++) {
    for (let i = 0; i < 12; i++) {
        ...
        NOTES.push({name: nn + oct, freq: 440 * Math.pow(2, (midi - 69) / 12), black: nn.includes('#')});
    }
}

export const noteByName: Record<string, NoteInfo> = Object.fromEntries(NOTES.map(n => [n.name, n]));
export const idxOfNote: Record<string, number> = Object.fromEntries(NOTES.map((n, i) => [n.name, i]));
