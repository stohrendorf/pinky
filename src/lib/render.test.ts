...
import {
    cancelExport, dismissExportError, exportProgress, exportWav, rendering, renderSongToWav
} from './render';
import {
    createTimingMap
} from './timing';
...
beforeEach(() => {
    ...
    audio.renderOffline.mockImplementation(async (_seconds: number, _rate: number, schedule: () => Promise<number>) => {
        await schedule();
        return {};
    });
});
...
describe('mixer WAV export integration', () => {
    ...
    it('uses the conductor timing map for loop duration', async () => {
        ...
        expect(audio.renderOffline.mock.calls[0][0]).toBeCloseTo(createTimingMap(p).secondsBetween(4, 24)
            + p.instruments[0].params.rel * 1.5 + 3 + mixerTailSeconds(p.mixer));
        expect(audio.renderOffline.mock.calls[0][0]).not.toBeCloseTo(20 * 60 / p.bpm / 4
            + p.instruments[0].params.rel * 1.5 + 3 + mixerTailSeconds(p.mixer));
    });
    ...
    it('observes external aborts and exposes errors for keyboard-triggered exports', async () => {
        ...
        controller.abort();
        await expect(renderSongToWav({signal: controller.signal})).rejects.toMatchObject({name: 'AbortError'});
        expect(audio.renderOffline).not.toHaveBeenCalled();
        audio.renderOffline.mockRejectedValueOnce(new Error('worklet failed'));
        expect(await exportWav()).toContain('worklet failed');
        expect(get(exportProgress)).toMatchObject({stage: 'error', error: 'Render failed: worklet failed'});
        expect(get(rendering)).toBe(false);
        dismissExportError();
        expect(get(exportProgress)).toBeNull();
        expect(await renderSongToWav()).toBeInstanceOf(Blob);
    ...
    it('exposes actual stage fractions without synthesizing an overall percentage', async () => {
        ...
        expect(updates).toEqual([
            ['preparing', null], ['preparing', null], ['scheduling', 0], ['scheduling', 0.5], ['scheduling', 1],
            ['rendering', 0], ['rendering', 0.25], ['rendering', 1], ['encoding', 0], ['encoding', 0.5], ['encoding', 1]
        ]);
    });
    ...
    it('reports stage-local estimates and starts fresh during encoding and a subsequent export', async () => {
        ...
        for (let attempt = 0; attempt < 2; attempt++) {
            ...
            expect(updates.map(state => state.stage)).toEqual([
                'preparing', 'rendering', 'rendering', 'encoding', 'encoding', 'encoding', 'encoding'
            ]);
    ...
    it.each(['rendering', 'encoding'])('honors an abort inside the %s callback before encoding or download continues', async stage => {
        project.set(newEmptyProject());
        const download = vi.fn();
        vi.stubGlobal('document', {createElement: download});
        audio.renderOffline.mockImplementation(async (_seconds, _rate, schedule: () => Promise<number>, _config,
