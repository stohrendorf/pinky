...
import { encodeWav, encodeWavAsync, type PcmSource } from "./wav";
...
describe("yielding WAV encoder", () => {
  it("preserves the original PCM bytes, interleaving and header across chunk boundaries", async () => {
    ...
    const actual = await encodeWavAsync(source, {
      onProgress: (value) => progress.push(value),
    });
    expect(await actual.arrayBuffer()).toEqual(
      await encodeWav(source).arrayBuffer(),
    );
  ...
  it("rejects an early abort before accessing channel samples", async () => {
    ...
    controller.abort();
    await expect(
      encodeWavAsync(pcm(8), { signal: controller.signal }),
    ).rejects.toMatchObject({
      name: "AbortError",
    });
  });

  it.each([32768 / 70000, 1])(
    "discards encoding cancelled at progress %s",
    async (threshold) => {
      const controller = new AbortController();
      const progress: number[] = [];
      await expect(
        encodeWavAsync(pcm(70000), {
