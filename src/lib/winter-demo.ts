...
const VIOLIN_PARTIALS = [1, 0.7, 0.55, 0.42, 0.33, 0.26, 0.2, 0.15].map(
  (level, index) => ({
    ratio: index + 1,
    level,
  }),
...
const SECTION_PARTIALS = [1, 0.65, 0.5, 0.38, 0.28, 0.2].map(
  (level, index) => ({
    ratio: index + 1,
    level,
  }),
...
const HARPSICHORD_PARTIALS = [0.8, 1, 0.85, 0.7, 0.6, 0.5].map(
  (level, index) => ({
    ratio: index + 1,
    level,
  }),
...
export function decodeWinterPart(packed: string): ScoreNote[] {
  ...
  const read = (): number => {
    ...
    return (
      WINTER_ALPHABET.indexOf(packed[pos++]) * 64 +
      WINTER_ALPHABET.indexOf(packed[pos++])
    );
  };
...
const isTrilled = (note: ScoreNote, bar: number): boolean =>
  TRILL_BARS.some(
    (range) => bar >= range.from && bar <= range.to && note.len >= range.len,
  );
...
export function performMovement(movement: Movement): Performed[] {
  ...
  const accent = (step: number): number =>
    step % movement.beat === 0 ? 1 : 0.92;
  const add = (
    voice: WinterVoice,
    note: ScoreNote,
    level: number,
...
function buildAutomation(sections: PlacedSection[]): AutomationLane[] {
  ...
  const at = (name: string): PlacedSection =>
...
export function buildWinterDemo(): Project {
  ...
  const tracks: Track[] = LANES.map(({ label, color }) => ({
    name: label,
    color,
  }));
  return mixDemo(
