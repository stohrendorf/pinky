...
import {
    importProject, initProject, project, selInstId, selPatId
} from './project';
import {
    isProjectId, PROJECT_FORMAT_VERSION
} from './types';
...
describe('project import', () => {
    it('initializes a complete default demo when no saved project is available', () => {
        ...
        expect(initial!.arrangement.every(clip => isProjectId(clip.id) && isProjectId(clip.patternId))).toBe(true);
    ...
    it('rejects a project from another format instead of migrating it implicitly', () => {
        ...
        expect(importProject(JSON.stringify({...current, formatVersion: PROJECT_FORMAT_VERSION - 1}))).toBe(false);
    });
});