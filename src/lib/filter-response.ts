import type { InstrumentParams, PartialSpec } from "./types";
...
const PINK_POLES = [0.99886, 0.99332, 0.969, 0.8665, 0.55, -0.7616];
const PINK_GAINS = [
...
export function quadraticFrequencySamples(
  points: number,
  upper: number,
  lower: number,
  anchors: readonly FrequencyAnchor[] = [],
): number[] {
...
function partialsFor(params: InstrumentParams): PartialSpec[] {
  ...
  return Array.from(
    { length: Math.max(0, Math.round(params.harm)) },
    (_, index) => {
...
function bandTerms(
  ...
  sin2W: number,
  sampleRate: number,
): { br: number; bi: number; ar: number; ai: number } {
  const w0 =
    (2 * Math.PI * Math.min(band.frequency, sampleRate * 0.49)) / sampleRate;
...
export function filterMagnitude(
  ...
  return filterMagnitudeForFundamental(
    params,
    fundamental,
    frequency,
    sampleRate,
...
function filterMagnitudeForFundamental(
  params: InstrumentParams,
  fundamental: number,
  frequency: number,
  sampleRate: number,
): number {
  return filterMagnitudeForBands(
    responseBands(params, fundamental, sampleRate),
    frequency,
    sampleRate,
  );
}

function filterMagnitudeForBands(
  bands: ResponseBand[],
  frequency: number,
  sampleRate: number,
): number {
...
function filterResponseForFundamental(
  params: InstrumentParams,
  fundamental: number,
  frequency: number,
  sampleRate: number,
): ComplexResponse {
  return filterResponseForBands(
    responseBands(params, fundamental, sampleRate),
    frequency,
    sampleRate,
  );
...
function filterResponseForBands(
  bands: ResponseBand[],
  frequency: number,
  sampleRate: number,
): ComplexResponse {
...
export function unisonFilterMagnitude(
  params: InstrumentParams,
  note: string,
  frequency: number,
  sampleRate = 48000,
  ...
  for (let index = 0; index < ranks; index++) {
    ...
    const ratio = Math.pow(2, (position * params.detune) / 2400);
    const response = filterResponseForFundamental(
      params,
      fundamental * ratio,
      frequency,
      sampleRate,
...
export function filterResponseCurve(
  ...
  const samples = Array.from(
    { length: count },
    (_, index) => lower * Math.pow(upper / lower, index / (count - 1)),
  );
  ...
  return samples
    .sort((a, b) => a - b)
    .filter(
      (frequency, index) => index === 0 || frequency !== samples[index - 1],
    )
    .map((frequency) => {
      const magnitude = unisonFilterMagnitude(
        params,
        note,
        frequency,
        sampleRate,
