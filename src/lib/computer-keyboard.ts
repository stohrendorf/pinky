...
export const COMPUTER_KEY_BINDINGS = [
  { code: "KeyZ", character: "z", semitone: 0 },
  { code: "KeyS", character: "s", semitone: 1 },
  { code: "KeyX", character: "x", semitone: 2 },
  { code: "KeyD", character: "d", semitone: 3 },
  { code: "KeyC", character: "c", semitone: 4 },
  { code: "KeyV", character: "v", semitone: 5 },
  { code: "KeyG", character: "g", semitone: 6 },
  { code: "KeyB", character: "b", semitone: 7 },
  { code: "KeyH", character: "h", semitone: 8 },
  { code: "KeyN", character: "n", semitone: 9 },
  { code: "KeyJ", character: "j", semitone: 10 },
  { code: "KeyM", character: "m", semitone: 11 },
  { code: "KeyQ", character: "q", semitone: 12 },
  { code: "Digit2", character: "2", semitone: 13 },
  { code: "KeyW", character: "w", semitone: 14 },
  { code: "Digit3", character: "3", semitone: 15 },
  { code: "KeyE", character: "e", semitone: 16 },
  { code: "KeyR", character: "r", semitone: 17 },
  { code: "Digit5", character: "5", semitone: 18 },
  { code: "KeyT", character: "t", semitone: 19 },
  { code: "Digit6", character: "6", semitone: 20 },
  { code: "KeyY", character: "y", semitone: 21 },
  { code: "Digit7", character: "7", semitone: 22 },
  { code: "KeyU", character: "u", semitone: 23 },
] as const;
...
export function bindingForCode(
  ...
  return {
    ...
    note:
      NOTE_NAMES[binding.semitone % 12] +
      (octave + Math.floor(binding.semitone / 12)),
  };
}

export function learnKeyboardLabel(
  ...
  if (
    !character ||
    !bindingByCode.has(code) ||
    labels.get(code) === character
  ) {
    return;
  }
...
export async function loadKeyboardLayout(
  ...
  const keyboard = (
    globalThis.navigator as Navigator & { keyboard?: NavigatorKeyboard }
  ).keyboard;
...
export class HeldNoteSources {
  ...
  hold(source: string, voice: HeldVoice): boolean {
    ...
    const alreadyHeld = [...this.bySource.values()].some((existing) =>
      sameVoice(existing, voice),
    );
  ...
  release(source: string): HeldVoice | null {
    ...
    return [...this.bySource.values()].some((existing) =>
      sameVoice(existing, voice),
    )
      ? null
      : voice;
