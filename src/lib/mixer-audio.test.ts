...
class Node {
  ...
  constructor(
    ...
    this.gain = new Param(
      Number(options.gain ?? (kind === "GainNode" ? 1 : 0)),
    );
...
function graph(meter = false) {
  ...
  const master = new Node(context, "master"),
    reverb = new Node(context, "reverb");
  const audio = new MixerAudio(
...
function strip(input: AudioNode | undefined) {
  ...
  const low = [...highpass.connections][0],
    high = [...low.connections][0];
  const volume = [...high.connections][0],
    pan = [...volume.connections][0],
    gate = [...pan.connections][0];
...
describe("persistent mixer audio routing", () => {
  it("routes live preview scope keys through their plain instrument mixer channel", () => {
    ...
    expect(tap.connections.has(audio.input("lead") as unknown as Node)).toBe(
      true,
    );
  ...
  it("schedules smooth strip ramps without reconfiguration and restores persisted values", () => {
    ...
    expect(a.volume.gain.linearRampToValueAtTime).toHaveBeenLastCalledWith(
      0.25,
      2.1,
    );
    expect(audio.automate("echo", "delayTime", 0.5, 2, 0.1)).toBe(true);
    expect(delay.delayTime.linearRampToValueAtTime).toHaveBeenLastCalledWith(
      0.5,
      2.1,
    );
  ...
  it("routes nested buses and post-fader sends; mute gates dry, reverb and every send together", () => {
    ...
    const a = strip(audio.input("a"));
    expect(
      [...a.gate.connections].some((edge) =>
        edge.connections.has(audio.input("group") as unknown as Node),
      ),
    ).toBe(true);
    expect(
      [...a.gate.connections].some(
        (edge) =>
          edge.gain.value === 0.3 &&
          edge.connections.has(audio.input("echo") as unknown as Node),
      ),
    ).toBe(true);
    expect(
      [...a.gate.connections].some((edge) => edge.connections.has(reverb)),
    ).toBe(true);
  ...
  it("uses delay-only wet returns and disconnects removed sends, buses and channels", () => {
    ...
    const feedback = [...delay.connections].find(
      (node) => node.kind === "GainNode",
    )!;
    ...
    expect(context.nodes.some((node) => node.connections.has(echo))).toBe(
      false,
    );
    audio.configure(undefined);
    expect(audio.input("a")).toBeUndefined();
    expect(context.nodes.some((node) => node.connections.has(master))).toBe(
      false,
    );
  });
  ...
  it("meters both stereo channels independently and allocates no analysers offline", () => {
    ...
    const analysers = context.nodes.filter(
      (node) => node.kind === "AnalyserNode",
    );
  ...
  it("bounds offline telemetry for long songs without changing live meters and drops messages after cleanup", () => {
    ...
    expect(node.port.postMessage).toHaveBeenLastCalledWith({
      type: "progress",
      intervalFrames: 144000,
    });
    const receive = node.port.onmessage as unknown as (event: {
      data: unknown;
    }) => void;
  ...
  it("loads protection once per context, propagates failure, and coalesces limiter updates", async () => {
    ...
    await expect(
      loadLimiter(new Context() as unknown as BaseAudioContext),
    ).rejects.toThrow("module failed");
  });
});
...
describe("engine graph isolation and latency trimming", () => {
  ...
  it("retains pre-init configuration, places protection last and does not double-feed reverb", async () => {
    ...
    expect(
      c.nodes
        .filter((n) => n.kind === "DynamicsCompressorNode")
        .every((n) => !n.connections.size),
    ).toBe(true);
  ...
  it("keeps the old sum-fed reverb and compressor for a missing mixer", async () => {
    ...
    await engine.ensureAudio();
    const c = Context.instances[0];
    const compressor = c.nodes.find(
      (n) => n.kind === "DynamicsCompressorNode",
    )!;
    const reverb = c.nodes.find((n) => n.kind === "ConvolverNode")!;
  ...
  it.each([false, true])(
    "restores live controls and graph after offline success/error (%s)",
    async (fail) => {
      const engine = await freshEngine();
      const mixer = createMixer(["a"]);
      engine.configureMixer(mixer, ["a"]);
  ...
  it("guards concurrent bounces and keeps project edits out of the offline graph", async () => {
    ...
    await started;
    await expect(
      engine.renderOffline(0.1, 48000, () => undefined),
    ...
    const limiter = offline.nodes.find(
      (node) => node.kind === "pinky-mixer-limiter",
    )!;
  ...
  it("restores after a rendering rejection and can render again", async () => {
    ...
    Context.render = null;
    expect(
      (await engine.renderOffline(0.1, 48000, () => undefined)).length,
    ).toBe(4800);
  });
  ...
  it("awaits asynchronous scheduling under offline guards and restores after an asynchronous failure", async () => {
    ...
    const analyser = engine.getAnalyser();
    await expect(
      engine.renderOffline(0.1, 48000, async () => {
  ...
  it("preserves limiter latency trimming across buffer-copy chunk boundaries", async () => {
    ...
    for (const channel of [0, 1]) {
      for (const frame of [0, 262143, 262144, 287999]) {
        expect(result.getChannelData(channel)[frame]).toBeCloseTo(
          (frame + 240) / 288240,
          6,
        );
      }
  ...
  it("aborts a pending worklet load without late graph construction, then allows live use and retry", async () => {
    ...
    const pending = engine.renderOffline(1, 48000, schedule, undefined, {
  ...
  it("keeps engine ownership until a continuous suspendable render finishes after cancellation", async () => {
    ...
    const pending = engine.renderOffline(1, 48000, () => undefined, undefined, {
      signal: controller.signal,
    });
    const rejected = expect(pending).rejects.toMatchObject({
      name: "AbortError",
    ...
    expect(offline.nodes.every((node) => node.connections.size === 0)).toBe(
      true,
    );
    expect(
      offline.nodes.find((node) => node.kind === "AudioBufferSourceNode")?.stop,
    ).toHaveBeenCalled();
    expect(
      offline.nodes.find((node) => node.kind === "pinky-mixer-limiter")?.port
        .close,
    ).toHaveBeenCalled();
    expect(engine.isRendering()).toBe(false);
    expect(engine.getAnalyser()).toBe(analyser);
  ...
  it("restores the live graph immediately and defers disposal after a non-suspendable render is cancelled", async () => {
    ...
    const pending = engine.renderOffline(
      1,
      48000,
      () => {
        Object.defineProperty(Context.instances[1], "suspend", {
          value: undefined,
        });
      },
      undefined,
    ...
    expect(limiter.port.postMessage).toHaveBeenCalledWith({
      type: "progress",
      intervalFrames: 12032,
    });
    const receive = limiter.port.onmessage as unknown as (event: {
      data: unknown;
    }) => void;
