...
describe('pattern note scheduling', () => {
    ...
    it('keeps one voice alive by scheduling each portamento transition at its source note', () => {
        ...
        expect(engine.noteOnAt).toHaveBeenCalledExactlyOnceWith('lead', 'C4', 1, lead.params, 1);
        expect(engine.glideAt).toHaveBeenNthCalledWith(1, 'lead', 'C4', 'D4', 1.4, 0.4, 'linear');
        ...
        expect(engine.glideAt).toHaveBeenNthCalledWith(2, 'lead', 'D4', 'E4', 2.2, 0.4, 'linear');
        expect(engine.noteOffAt).not.toHaveBeenCalled();
