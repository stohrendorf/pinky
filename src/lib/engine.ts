...
function copyMixer(
  ...
  return {
    channels: Object.fromEntries(
      Object.entries(resolved.channels).map(([id, value]) => [
        id,
        channel(value),
      ]),
    ),
...
export function configureMixer(
  mixer: MixerState | undefined,
  instrumentIds: string[],
): void {
  ...
  for (const id of ["vol", "rev", "tilt"] as const) {
    if (master[id] !== values[id]) {
      liveMasterControls.apply(id, values[id]);
    }
  }
  if (liveGraph) {
    configureGraphMixer(liveGraph, snapshot, liveMasterControls);
  }
}

export function mixerMeters(): {
  master: MasterMeter;
  channels: Record<string, ChannelMeter>;
} {
  return {
    master: liveGraph?.limiter?.meters() ?? {
      peak: [0, 0],
      rms: [0, 0],
      reduction: 0,
    },
...
export function resetMixer(): void {
  if (mixerConfig.mixer) {
    liveGraph?.mixer?.reset(mixerConfig.mixer);
  }
}
...
function sweepTeardowns(now: number): void {
  ...
  let budget =
    SWEEP_BUDGET * Math.min(8, 1 + Math.floor(teardowns.length / 50));
...
function scheduleTeardown(t: Teardown): void {
  if (offline) {
    return;
  } // the offline graph dies together with its context
  teardowns.push(t);
  startHousekeeping();
}
...
function flattenLater(
  prm: AudioParam,
  v: number,
  at: number,
  forceLive = false,
): void {
  if (offline && !forceLive) {
    return;
...
function startHousekeeping(): void {
  if (sweeper === null) {
    sweeper = setInterval(housekeeping, SWEEP_MS);
  }
}
...
function housekeeping(): void {
  if (offline) {
    return;
  } // never recycle live nodes against the offline graph
...
function pooling(): boolean {
  ...
  nodePool.attach(ctx, engine.flushBus, () =>
    liveCtx ? liveCtx.currentTime : 0,
  );
  return true;
}
...
function takeGain(gain: number): GainNode {
  ...
  return pooling()
    ? nodePool.takeGain(gain, context)
    : new GainNode(context, { gain });
}
...
function takePanner(pan: number): StereoPannerNode {
  const context = ctx as BaseAudioContext;
  return pooling()
    ? nodePool.takePanner(pan, context)
    : new StereoPannerNode(context, { pan });
}
...
export function engineLoad(): {
  ...
  let nodes = 0;
  for (const lv of voiceCollection.liveVoices) {
    nodes += lv.voice.cost;
  }
  ...
  return {
    nodes,
    voices: voiceCollection.liveCount,
    budget: nodeBudget,
    pooled: pooledNodes(),
  };
}
...
export const masterState = (): { vol: number; tilt: number } =>
  liveMasterControls.state();
...
function reverbImpulse(seconds: number, decay: number): AudioBuffer {
  ...
  for (let ch = 0; ch < 2; ch++) {
    const d = buf.getChannelData(ch);
    for (let i = 0; i < len; i++) {
      d[i] = (audioRandom() * 2 - 1) * Math.pow(1 - i / len, decay);
    }
  }
  return buf;
}
...
async function buildGraph(
  ...
  const settings = config.mixer?.master ?? {
    ...resolveMixer(undefined, []).master,
    ...values,
  };
...
function configureGraphMixer(
  ...
  if (mixer && !initial) {
    for (const id of ["vol", "rev", "tilt"] as const) {
      if (o.mixerMaster?.[id] !== settings[id]) {
        controls.apply(id, settings[id]);
      }
    }
  }
...
function makeRank(
  ...
  if (!used.length && partials.length) {
    used = [partials[0]];
  } // never a silent voice
  ...
  if (p.noise > 0) {
    ...
    if (nbFrom !== p.noiseFreq) {
      ...
      nb.frequency.exponentialRampToValueAtTime(
        p.noiseFreq,
        when + p.pitchTime,
      );
  ...
  if (vibRatio > 0 && harmBands.length) {
    ...
    if (vibSource) {
      ...
      for (const { bq, r } of harmBands) {
        ...
        if (onset > 0.005) {
          g.gain.linearRampToValueAtTime(amp, when + onset);
        } else {
  ...
  if (Math.abs(panWanted) > 0.002) {
    insertPan(panWanted + (audioRandom() - 0.5) * 0.06);
  } else {
    env.connect(voiceOutput);
  }
  ...
  flattenLater(
    env.gain,
    p.sus * peak,
    when + p.att + Math.max(0.05, p.dec * 2),
  );
  ...
  const baseFreqAt = (t: number): number => {
    if (bendTime <= 0 || t >= bendStart + bendTime) {
      return bendTarget;
    }
    if (t <= bendStart) {
      return bendFrom;
    ...
    return (
      bendFrom * Math.pow(bendTarget / bendFrom, (t - bendStart) / bendTime)
    );
  };
  ...
  const voice: Voice = {
    ...
    glide(
      ...
      const k =
        bendTime > 0
          ? Math.min(1, Math.max(0, (at - bendStart) / bendTime))
          : 1;
      ...
      if (noiseBand && p.noiseBend > 0) {
        const nbCur =
          noiseBand.from * Math.pow(noiseBand.target / noiseBand.from, k);
      ...
      const g = env.gain as AudioParam & {
...
function makeVoice(
  ...
  const voice: Voice = {
    ...
    loudness: (at: number) =>
      ranks.reduce((m, r) => Math.max(m, r.loudness(at)), 0),
    stop(at: number) {
      voice.stopAt = Math.min(voice.stopAt, at);
...
function whileScheduling<T extends unknown[]>(
  fn: (...args: T) => void,
): (...args: T) => void {
  return (...args) => {
    if (!rendering || schedulingOffline) {
      fn(...args);
    }
  };
}

export const noteOnAt = whileScheduling(
  noteScheduler.noteOnAt.bind(noteScheduler),
);
...
export const noteOffAt = whileScheduling(
  noteScheduler.noteOffAt.bind(noteScheduler),
);
export const glideAt = (
  ...args: Parameters<typeof noteScheduler.glideAt>
): boolean =>
  (!rendering || schedulingOffline) && noteScheduler.glideAt(...args);
export const noteOff = whileScheduling(
  noteScheduler.noteOff.bind(noteScheduler),
);
export const allNotesOff = whileScheduling(
  noteScheduler.allNotesOff.bind(noteScheduler),
);
...
export const automateMaster = (
  id: string,
  value: number,
  at: number,
  ramp: number,
): void => selectedMasterControls().automate(id, value, at, ramp);
...
export async function renderOffline(
  ...
  try {
    ...
    try {
      ...
      if (!abandoned) {
        disposeOfflineGraph(offlineEngine ?? engine);
      }
      ctx = savedCtx;
      offline = false;
      engine = savedEngine;
