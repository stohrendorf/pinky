...
export type NameTreeEntry<T extends NamedTreeItem> =
  NameTreeFolderEntry | NameTreeItemEntry<T>;
...
function createFolder<T extends NamedTreeItem>(
  label: string,
  path: string,
): NameTreeFolder<T> {
...
export function flattenNameTree<T extends NamedTreeItem>(
  ...
  const countItems = (folder: NameTreeFolder<T>): number =>
    folder.items.length +
    folder.folders.reduce((total, child) => total + countItems(child), 0);
  const visit = (
    folder: NameTreeFolder<T>,
    depth: number,
    ancestors: string[],
  ) => {
    if (sortFolders) {
