...
describe('bundled demo hierarchy', () => {
    it.each(DEMO_SONGS)('organizes %s instruments and patterns into slash paths', song => {
        ...
        expect(demo.arrangement.every(({patternId}) => demo.patterns.some(({id}) => id === patternId))).toBe(true);
    });
    ...
    it('uses production-style sound families and song-form folders', () => {
        ...
        expect(toccata.instruments.some(({name}) => name === 'Orchestra/Strings/Cello')).toBe(true);
        expect(toccata.patterns.find(({name}) => name.endsWith('/finale spin'))?.name).toBe('04 Climax/finale spin');
    });

    it.each(DEMO_SONGS)('keeps every %s arranger track occupied and referenced', song => {
        const demo = buildDemoProject(song);

        expect(demo.tracks.every((_, index) => demo.arrangement.some(clip => clip.track === index))).toBe(true);
