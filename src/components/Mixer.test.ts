...
interface MixerActions {
  ...
  masterNumber: (
    key: Exclude<keyof MixerMaster, "limiter">,
    value: number,
  ) => void;
...
describe("Mixer component controls", () => {
  ...
  it("keeps compact labelled strip controls available before selection", () => {
    ...
    expect(mixer).toMatch(
      /onMount\(\(\) => \{[\s\S]*setInterval[\s\S]*1000 \/ 15/,
    );
  ...
  it("makes channel roles, signal flow and the selected inspector immediately clear", () => {
    ...
    expect(mixer).toContain(
      "Select a strip to edit routing, sends and processing.",
    );
  ...
  it("shares bounded vertical faders and stereo sample meters with Master", () => {
    ...
    const actions = handlers<{
      change: (input: { valueAsNumber: number }) => void;
    }>(fader, { max: 1, onchange: (value: number) => values.push(value) }, [
      "change",
    ]);
  ...
  it("supports nested routes but rejects output and send feedback, including muted zero sends", () => {
    ...
    expect(p.mixer!.channels[id].sends).toEqual([
      { busId: delay.id, level: 0.25 },
    ]);
    expect(mixer).toMatch(
  ...
  it("makes TopBar master controls project-backed, bounded and rendering-safe", () => {
    ...
    const actions = handlers<{
      setMaster: (key: string, value: number) => void;
    }>(toolbar, scope, ["setMaster"]);
