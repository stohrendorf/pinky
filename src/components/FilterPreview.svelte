<script lang="ts">
    ...
    import { audioSampleRate } from '../lib/engine';
    import { filterResponseCurve } from '../lib/filter-response';
    ...
    const response = $derived(
        filterResponseCurve(params, note, RESPONSE_POINTS, audioSampleRate()),
    );
    ...
    const frequencyX = $derived(
        (frequency: number) =>
            PAD_X +
            (WIDTH - PAD_X * 2) * clamp(Math.log(frequency / minFrequency) / frequencyRange, 0, 1),
    );
    ...
    const yForDb = $derived(
        (value: number) =>
            HEIGHT - PAD_Y - clamp((value - minDb) / dbRange, 0, 1) * (HEIGHT - PAD_Y * 2),
    );
    const path = $derived(
        response
            .map(point => {
...
<section class="filter-preview" aria-label="Filter response preview">
    ...
    >
        <line class="grid" x1={PAD_X} x2={WIDTH - PAD_X} y1={HEIGHT / 4} y2={HEIGHT / 4} />
        <line class="grid" x1={PAD_X} x2={WIDTH - PAD_X} y1={HEIGHT / 2} y2={HEIGHT / 2} />
        <line
            class="grid"
            x1={PAD_X}
            x2={WIDTH - PAD_X}
        ...
        <text text-anchor="end" x={WIDTH - PAD_X} y={HEIGHT - 2}
            >{formatFrequency(maxFrequency)}</text
        >
    </svg>
</section>
