...
describe('TopBar demo selection', () => {
    ...
    it('groups session controls separately from utility controls', () => {
        ...
        expect(toolbar).toMatch(
            /class="utility-sidebar"[\s\S]*class="sidebar-column"[\s\S]*class="master-controls"[\s\S]*aria-label="Timing controls"/,
        );
    ...
    it('links subtly to the source repository from the persistent header', () => {
        expect(toolbar).toMatch(
            /<a[\s\S]*class="repo-link"[\s\S]*href="https:\/\/github\.com\/stohrendorf\/pinky"/,
        );
    ...
    it('keeps the top bar groups shrinkable within the viewport', () => {
        expect(toolbar).toMatch(
            /\.topbar-main\s*\{[\s\S]*min-width:\s*0;[\s\S]*overflow:\s*hidden;/s,
        );
    ...
    it('keeps save confirmation persistent while placing the save action in project utilities', () => {
        const saveAction = components(componentMarkup(toolbar), 'Button').find(
            button => textContent(button) === 'Save',
        );

        expect(toolbar).toMatch(
            /<span class="song-label">\{\$songLabel\}<\/span>\s*<span class="saved-flash" aria-live="polite">\{saved\}<\/span>/,
        );
    ...
    it('opens one accessible sidebar without utility sub-navigation', () => {
        const utilities = elements(componentMarkup(toolbar), 'aside').find(node =>
            hasAttribute(node, 'id', 'topbar-utilities'),
        );

        expect(toolbar).toContain('let utilityExpanded = $state(false)');
        expect(toolbar).toContain('function toggleUtilities');
        expect(utilities).toBeDefined();
        expect(hasAttribute(utilities!, 'class', 'utility-sidebar')).toBe(true);
        ...
        expect(toolbar).toMatch(
            /\.utility-sidebar\s*\{[\s\S]*max-height:\s*calc\(100dvh - 56px\);/s,
        );
    });

    it('uses the Pinky brand as the accessible application-menu button', () => {
        expect(toolbar).toMatch(
            /<button[\s\S]*class="brand"[\s\S]*aria-controls="topbar-utilities"[\s\S]*aria-expanded=\{utilityExpanded\}/,
        );
        ...
        expect(toolbar).toMatch(
            /\.brand\s*\{[\s\S]*border:\s*1px solid var\(--border\);[\s\S]*background:\s*var\(--color-surface-raised\);/s,
        );
    ...
    it('keeps the sidebar compact and orders sections by relevance', () => {
        ...
        expect(toolbar).toMatch(
            /<span class="menu-heading">Project<\/span>[\s\S]*<span class="menu-heading">Render<\/span>/,
        );
        expect(toolbar).toMatch(
            /<span class="menu-heading">Render<\/span>[\s\S]*<span class="menu-heading">Mix<\/span>[\s\S]*<span class="menu-heading">Timing<\/span>/,
        );
