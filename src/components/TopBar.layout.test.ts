...
describe('TopBar desktop layout', () => {
    it('offers the shared demo registry with current-song state and descriptions on hover', () => {
        ...
        expect(
            components(markup, 'Dialog').some(node => hasAttribute(node, 'title', 'Demo songs')),
        ).toBe(false);
    });
    ...
    it('keeps a single compact desktop row rather than wrapping into a mobile header', () => {
        ...
        for (const selector of ['.topbar-main', '.session-group', '.transport-controls']) {
    ...
    it('has only one Save action and announces confirmation without reserving header space', () => {
        const saves = components(markup, 'Button').filter(node => textContent(node) === 'Save');
        expect(saves).toHaveLength(1);
        expect(
            elements([saves[0]], 'i').map(node => hasAttribute(node, 'aria-hidden', 'true')),
        ).toEqual([true, true]);
        ...
        expect(styleRules(toolbar).get('.saved-flash')?.get('position')).toBe('absolute');
    });
    ...
    it('uses named nonmodal panels with no nested settings or permanent help paragraphs', () => {
        ...
        expect(styleRules(toolbar).get('.toolbar-panel')?.get('overflow')).toBe('auto');
    });
    ...
    it('keeps master and performance controls in the flat Audio panel', () => {
        ...
        expect(elements([audio], 'h3').map(textContent)).toEqual(['Master', 'Performance']);
        expect(components([audio], 'Slider')).toHaveLength(1);
        expect(hasAttribute(elements([audio], 'fieldset')[0], 'disabled')).toBe(true);
        expect(elements([audio], 'input').some(node => hasAttribute(node, 'type', 'range'))).toBe(
            true,
        );
    });

    it('keeps Mixer and shortcut help directly available', () => {
        const mixer = components(markup, 'Button').find(node => textContent(node) === 'Mixer')!;
        expect(
            elements([mixer], 'i').some(node => hasAttribute(node, 'class', 'fa fa-chart-simple')),
        ).toBe(true);
        expect(
