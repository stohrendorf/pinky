...
import { componentFunction, componentSource } from '../../test/svelte-semantics';

const source = componentSource(new URL('./Dialog.svelte', import.meta.url));

function fixture() {
    const first = { matches: () => false, checkVisibility: () => true, focus: vi.fn() };
    ...
    const dialogEl = { querySelectorAll: () => [disabled, first, last, hidden], focus: vi.fn() };
    const document: { activeElement: unknown } = { activeElement: last };
    const close = vi.fn();
    const handle = componentFunction<(event: object) => void>(source, 'handleKey', {
        dialogEl,
        document,
        close,
    });
