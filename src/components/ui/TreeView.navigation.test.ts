...
function fixture() {
    return {
        entries: [
            ...
            { kind: 'item', item: { id: 'beat' }, ancestors: ['02 Main', '02 Main/Drums'] },
        ],
...
describe('TreeView folder navigation', () => {
    it('collapses and expands all folders without changing the selected item', () => {
        const scope = fixture();
        const collapse = componentFunction<() => void>(source, 'collapseAll', scope);
        const expand = componentFunction<() => void>(source, 'expandAll', scope);
        collapse();
        expect([...scope.collapsedFolders]).toEqual(['02 Main', '02 Main/Drums', '03 Outro']);
        expand();
    ...
    it('provides names for the tree and folder tools, and full paths for truncated rows', () => {
        ...
        const tree = elements(markup, 'div').find(node => hasAttribute(node, 'role', 'tree'))!;
        expect(hasAttribute(tree, 'aria-label')).toBe(true);
        const buttons = elements(markup, 'button');
        expect(buttons.some(node => hasAttribute(node, 'aria-label', 'Collapse all folders'))).toBe(
            true,
        );
        expect(buttons.some(node => hasAttribute(node, 'aria-label', 'Expand all folders'))).toBe(
            true,
        );
        const rows = buttons.filter(node => hasAttribute(node, 'class', 'row-main'));
