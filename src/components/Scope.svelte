<script lang="ts">
    ...
    function chainResponse(
        ...
        for (let k = 0, o = 0; k < nPre; k++, o += BAND_C) {
            const b1 = t[o + 1],
                ...
                a1 = t[o + 3],
                a2 = t[o + 4];
            const br = t[o] + b1 * cw + b2 * c2w,
                bi = -(b1 * sw + b2 * s2w);
            const ar = 1 + a1 * cw + a2 * c2w,
                ai = -(a1 * sw + a2 * s2w);
    ...
    onMount(() => {
        ...
        const cwT = new Float64Array(CURVE_CAPACITY),
            c2wT = new Float64Array(CURVE_CAPACITY);
        const swT = new Float64Array(CURVE_CAPACITY),
            s2wT = new Float64Array(CURVE_CAPACITY);
        ...
        const draw = (now: number) => {
            ...
            const dbMin = analyser.minDecibels,
                dbSpan = Math.max(1, analyser.maxDecibels - dbMin);
                ...
                if (voices.length) {
                    ...
                    for (const v of voices) {
                        ...
                        for (const post of [false, true]) {
                            for (const b of v.bands) {
                                ...
                                if (
                                    Math.abs(b.gain) < MIN_GAIN_DB ||
                                    (n + 1) * BAND_C > ch.terms.length
                                ) {
                                    continue;
                                }
                    ...
                    for (let i = 0; i < curvePoints; i++) {
                        ...
                        pow *=
                            pinkT[i] *
                            gMaster *
                            termsAt(tilt, 0, cw, c2w) *
                            termsAt(tilt, 6, cw, c2w);
                    ...
                    for (let j = 0; j < nv; j++) {
                        ...
                        for (const b of ch.bands) {
                            ...
                            dots.push({
                                x,
                                y: dbToY(10 * Math.log10(Math.max(1e-12, pow)) + cal),
                                color,
                                alpha,
                            });
                        }
            ...
            g2d.fillText(
                `${load.voices} voices · ${load.nodes} / ${load.budget} nodes · ${load.pooled} pooled`,
                8,
                19,
            );
