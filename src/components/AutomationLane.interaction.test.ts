...
const automationLane = readFileSync(fileURLToPath(new URL('./AutomationLane.svelte', import.meta.url)), 'utf8');
...
describe('AutomationLane selection workflow', () => {
    it('clears the arranger-wide selected point before adding another point', () => {
        ...
        expect(automationLane.indexOf('if (hasSelectedPoint) {')).toBeLessThan(automationLane.indexOf('const pt: AutomationPoint'));
    });
});
