...
describe('Conductor component', () => {
    it('compiles labelled controls and pointer interaction without accessibility warnings', () => {
        ...
        expect(
            compile(playlist, { filename: 'Playlist.svelte', generate: 'client' }).js.code.length,
        ).toBeGreaterThan(0);
    });
    ...
    it('loads and saves title, tempo/ramp and signature together in one edit, preserving IDs and music', () => {
        ...
        expect([scope.sectionName, scope.tempoBpm, scope.tempoCurve, scope.signature]).toEqual([
            'Rain',
            90,
            'linear',
            '7/8',
    ...
    it('traps focus, filters hidden/disabled controls and shields input from app shortcuts', () => {
        ...
        const first = control(),
            last = control();
        const dialog = {
            querySelectorAll: () => [first, control(true), control(false, false), last],
            focus: vi.fn(),
        };
    ...
    it('retains actual mixed-meter bar boundaries and useful position labels', () => {
        ...
        expect(playlist).toMatch(/barsInRange\(\s*\$project,\s*scrollLeft\s*\/\s*cellWidth,/);
