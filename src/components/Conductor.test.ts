...
import { barAt, barsInRange, createTimingMap } from "../lib/timing";
import { styleRules } from "../test/svelte-semantics";

const source = readFileSync(
  fileURLToPath(new URL("./Conductor.svelte", import.meta.url)),
  "utf8",
);
...
function fixture(grouped = false) {
  const p: Project = {
    ...
    tracks: [],
    arrangement: [
      { id: randomUUID(), patternId: "polymeter", start: 0, len: 96, track: 0 },
    ],
  ...
  const scope = {
    ...
    get conductor() {
      return (
        this.$project?.conductor ?? { tempos: [], meters: [], sections: [] }
      );
    },
    get points() {
    ...
    markerStep: 0,
    tempoBpm: undefined as number | undefined,
    tempoCurve: "hold",
    signature: "",
    sectionName: "",
    error: "",
    ...
    editorProject: null as Project | null,
    editorSnapshot: "",
    opener: { focus: vi.fn(), isConnected: true },
    document: { activeElement: null as unknown, querySelector: vi.fn() },
    HTMLElement: class {},
    touch: vi.fn(),
    markerPoints,
    moveMarkerAt,
...
describe("Conductor component", () => {
  it("compiles labelled controls and pointer interaction without accessibility warnings", () => {
    ...
    expect(
      compile(playlist, { filename: "Playlist.svelte", generate: "client" }).js
        .code.length,
    ).toBeGreaterThan(0);
  });
  ...
  it("keeps one compact strip and all fields together, without numeric position or type pickers", () => {
    ...
    expect(source).toMatch(/<label\s*>\s*Title/);
    expect(source).toMatch(/<label\s*>\s*BPM/);
    expect(source).toMatch(/<label\s*>\s*Time signature/);
    for (const text of [
      "Exact step",
      "Use cursor",
      "Marker type",
    ...
    expect(source).toMatch(
      /<div class="conductor-label">\s*<span>Markers<\/span>\s*<\/div>/,
    );
    ...
    expect(styleRules(source).get(".conductor-viewport")?.get("overflow")).toBe(
      "clip",
    );
  });
  ...
  it("adds an immediately draggable placeholder at the cursor without opening a dialog or changing timing", () => {
    ...
    actions.newMarker();
    expect(p.conductor?.sections[0]).toMatchObject({
      step: 19,
      name: "Marker",
    });
  ...
  it("loads and saves title, tempo/ramp and signature together in one edit, preserving IDs and music", () => {
    ...
    expect(scope.points[0]).toMatchObject({
      id: ids.id,
      section: { id: ids.section!.id, name: "Bronze" },
      tempo: { id: ids.tempo!.id, bpm: 108, curve: "linear" },
  ...
  it.each([
    { tempoBpm: 29 },
    { signature: "7/3" },
    { sectionName: "x".repeat(81) },
    { tempoCurve: "exponential" },
  ])("rejects an invalid field atomically: %j", (invalid) => {
    const { p, scope, actions } = fixture(true);
    actions.editMarker(scope.points[0].id);
  ...
  it("moves the entire group only on drop and suppresses the following click, not the next deliberate click", () => {
    ...
    expect(
      (down.currentTarget as HTMLElement).setPointerCapture,
  ...
  it.each(["escape", "cancel"] as const)(
    "cancels a drag via %s without mutating the project",
    (how) => {
      const { p, scope, actions } = fixture(true);
      const before = JSON.stringify(p);
      const point = scope.points[0];
  ...
  it.each(["$playing", "$rendering"] as const)(
    "blocks mutations if %s changes during editing/dragging",
    (lock) => {
      const { p, scope, actions } = fixture(true);
      const point = scope.points[0];
  ...
  it.each(["deleted", "replaced"] as const)(
    "does not resurrect stale markers when the project is %s",
    (change) => {
      const { p, scope, actions } = fixture(true);
      const point = scope.points[0];
      actions.editMarker(point.id);
  ...
  it("closes on Escape with focus restoration and removes dialog listeners", async () => {
    ...
    const action = actions.dialogKeyboard({
      closest: () => dialog,
    } as unknown as HTMLElement);
    expect(dialog.addEventListener).toHaveBeenCalledWith(
      "keydown",
      expect.any(Function),
    );
    scope.show = true;
    ...
    action.destroy();
    expect(dialog.removeEventListener).toHaveBeenCalledWith(
      "keydown",
      dialog.addEventListener.mock.calls[0][1],
    );
  });
  ...
  it("retains actual mixed-meter bar boundaries and useful position labels", () => {
    ...
    expect(
      barsInRange(p, 0, 39).map((bar) => [bar.bar, bar.start, bar.end]),
    ).toEqual([
      [1, 0, 10],
      [2, 10, 24],
      [3, 24, 38],
    ...
    expect(playlist).toMatch(
      /barsInRange\(\s*\$project,\s*scrollLeft\s*\/\s*cellWidth,/,
    );
