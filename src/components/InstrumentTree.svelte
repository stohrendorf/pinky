<script lang="ts">
    ...
    function action(actionId: string, item: NamedTreeItem) {
        ...
        if (actionId === 'delete' && ($project?.instruments.length || 0) > 1) {showDelete = true;}
    }
    ...
    function remove() {
        if (!$project || !target || $project.instruments.length <= 1) {return;}
        const id = target.id;
    ...
    function folderAction(actionId: string, path: string) {
        if (actionId === 'rename') {renameFolder(path);}
    }
    ...
    function submitRename(e: CustomEvent<string>) {
        ...
        if (folderToRename) {
            ...
            $project?.instruments.forEach(instrument => {
                if (instrument.name === folderToRename) {instrument.name = e.detail;}
                else if (instrument.name.startsWith(prefix)) {instrument.name = e.detail + instrument.name.slice(folderToRename!.length);}
            });
            folderToRename = null;
            touch();
        } else {rename(e);}
