<script lang="ts">
    ...
    function newMarker(at?: number) {
        ...
        if (existing) {focusMarker(existing.id); return;}
        try {
            const data = updateMarkerAt(conductor, step, {name: 'Marker', bpm: undefined, curve: 'hold', signature: ''});
            $project.conductor = data;
            touch();
            focusMarker(markerPoints(data).find(point => point.step === step)!.id);
            laneMessage = '';
        } catch (cause) {laneMessage = (cause as Error).message;}
    }
    ...
    function deleteMarker() {
        if (!$project || $playing || $rendering || !editorIsCurrent()) {return;}
    ...
    function startDrag(event: PointerEvent, point: MarkerPoint) {
        if (event.button !== 0 || !$project || $playing || $rendering) {return;}
        event.stopPropagation();
        suppressClick = false;
        selectedId = point.id;
        laneMessage = '';
        dragProject = $project;
        drag = {id: point.id, pointer: event.pointerId, x: event.clientX, scroll: scrollLeft,
    ...
    function updateDrag(event: PointerEvent) {
        ...
        try {moveMarkerAt(conductor, drag.from, drag.to); drag.problem = '';}
        catch (cause) {drag.problem = (cause as Error).message;}
    ...
    function finishDrag(event: PointerEvent) {
        ...
        const finished = drag;
        drag = null;
        if (finished?.moved && !finished.problem) {movePoint(finished.id, finished.to);}
    }
...
bind:value={tempoBpm}/>
                        ...
                        <label>Time signature
                            <input placeholder={`${inheritedMeter.numerator}/${inheritedMeter.denominator}`} type="text" bind:value={signature}/>
                        </label>
                    </div>
...
<style>
    ...
    .marker:hover, .marker:focus-within, .marker.dragging {max-width: none !important; z-index: 2;}
    .marker button {min-width: 0; padding: 2px 6px; border: 0; border-radius: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; text-align: left; cursor: grab; touch-action: none; user-select: none;}
    .marker.dragging button {cursor: grabbing;}
    .marker.selected {outline: 1px solid var(--accent2); outline-offset: -1px;}
    .marker.invalid {border-color: var(--accent); outline-color: var(--accent);}
    ...
    input {min-width: 0; width: 100%; padding: 7px;}
    .check {flex-direction: row; align-items: center; font-size: 11px;}
    .check input {width: auto; margin: 0;}
    .timing-fields {display: grid; grid-template-columns: 1fr 1fr; gap: 10px;}
    ...
    .hint {font-size: 11px; color: var(--secondary-text); margin-bottom: 0;}
    .lane-status {position: absolute; width: 1px; height: 1px; overflow: hidden; clip-path: inset(50%);}
    .lane-status.drag-feedback {clip-path: none; width: auto; height: auto; right: 8px; top: 100%; padding: 5px 8px; background: var(--color-surface); border: 1px solid var(--border); font-size: 11px; z-index: 3; pointer-events: none;}
</style>