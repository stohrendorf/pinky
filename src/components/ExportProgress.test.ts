...
describe('export progress modal', () => {
    ...
    it('distinguishes waiting for telemetry from graph initialization', () => {
        expect(
            markup({ stage: 'rendering', progress: null, cancelling: false, canSuspend: false }),
        ).toContain('Waiting for audio progress…');
        expect(markup({ stage: 'preparing', progress: null, cancelling: false })).toContain(
    ...
    it('labels the ETA as approximate and stage-local and hides it during cancellation or errors', () => {
        ...
        expect(markup({ ...state, stage: 'error', error: 'Render failed' })).not.toContain(
            'left in this stage',
        );
