...
const bar = readFileSync(fileURLToPath(new URL('./InstrumentPresetBar.svelte', import.meta.url)), 'utf8');
...
describe('InstrumentPresetBar', () => {
    it('keeps the preset picker and its commands compact enough for the editor title bar', () => {
        ...
        expect(bar).toMatch(/<select[\s\S]*aria-label="Select preset"[\s\S]*bind:value=\{presetName\}/);
