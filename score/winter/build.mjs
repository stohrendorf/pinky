...
function parseMidi(bytes) {
  ...
  const chunk = () => ({
    type: String.fromCharCode(...bytes.subarray(pos, (pos += 4))),
    len: u32(),
  });
  const header = chunk();
  if (header.type !== "MThd") {
  ...
  for (let t = 0; t < nTracks; t++) {
    ...
    while (pos < end) {
      ...
      if (status === 0xff) {
        ...
        const len = vlq();
        if (meta === 0x03) {
          track.name = String.fromCharCode(...bytes.subarray(pos, pos + len));
        }
        pos += len;
...
function packValue(v) {
  if (v < 0 || v >= 4096 || v !== Math.floor(v)) {
    throw new Error(`cannot pack ${v}`);
  }
...
function packPart(notes) {
  ...
  for (const n of notes) {
    s +=
      packValue(n.start - last) +
      packValue(n.len) +
      packValue(n.midi - PITCH_BASE);
    last = n.start;
  }
  return s;
}
...
function convert(movement) {
  ...
  if (midi.division !== 384) {
    throw new Error(
      `${movement.file}: expected 384 ticks per quarter, got ${midi.division}`,
    );
  }
  const parts = {};
  const stats = [];
  for (const track of midi.tracks) {
    ...
    for (const raw of track.notes) {
      ...
      if (raw.tick % movement.ticksPerStep || raw.dur % movement.ticksPerStep) {
        offGrid++;
      }
      const key = `${start}/${raw.midi}`;
      const seen = byKey.get(key);
      if (seen) {
        seen.len = Math.max(seen.len, len);
      } else {
        byKey.set(key, { start, len, midi: raw.midi });
      }
    }
    const notes = [...byKey.values()].sort(
    ...
    if (lastStep > movement.bars * movement.stepsPerBar) {
      throw new Error(
        `${movement.file}/${part}: ends at step ${lastStep}, past bar ${movement.bars}`,
      );
    }
    parts[part] = packPart(notes);
    stats.push(
      `${part} ${notes.length} notes${offGrid ? ` (${offGrid} quantised)` : ""}`,
    );
  }
  for (const part of Object.values(PART_OF_TRACK)) {
    if (!parts[part]) {
      throw new Error(`${movement.file}: no track for ${part}`);
    }
  }
  return { parts, stats };
}
...
for (const m of movements) {
  console.log(`${m.title}: ${m.stats.join(", ")}`);
}
console.log(`-> ${out}`);
